...
inline auto screen_to_ground(const Game::Systems::PickingService* picking_service,
  ...
  if ((window == nullptr) || (camera == nullptr) || (picking_service == nullptr)) {
    return false;
  }
  ...
  return Game::Systems::PickingService::screen_to_ground(
      *camera, w, h, screen_pt, out_world);
}
...
inline auto world_to_screen(const Game::Systems::PickingService* picking_service,
  ...
  int const w = (viewport_width > 0 ? viewport_width : window->width());
  int const h = (viewport_height > 0 ? viewport_height : window->height());
  return Game::Systems::PickingService::world_to_screen(
      *camera, w, h, world, out_screen);
}
