...
| `sfx/combat/masonry_hit_01..03.ogg`      | 0.35 s | A sword or spear striking dressed stone, a hard dry crack with grit shaking loose, close, no ring or reverb | `combat.hit.structure`, which currently borrows `sfx.combat.stone_impact_01` from `combat.hit.siege`; the two share one recording, so a catapult hitting a wall sounds exactly like a swordsman doing it, and whichever fires first puts the other on that resource's cooldown |
