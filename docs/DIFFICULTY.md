...
- `tests/core/difficulty_presets_test.cpp` — a real mission loaded at each preset, checking that every enemy category moves together, that the player's force and treasury never move, that commanders are never duplicated, and that Normal reproduces the authored match exactly.
