...
`FarmWorkerVisual` (`register_farm_worker_visual`): the civilian visual spec,
...
Registration then derives four archetypes by hanging the shared field props
(`render/entity/farm_worker_props.cpp`) on those two: a straw sun hat for
tending and reaping, hat plus a tied wheat sheaf for gathering, and a hat tipped
forward over the face for the sleeper. A straw hat is field dress, not faction
dress, so both nations share it. The props are registered as ordinary humanoid
equipment contributions, and head gear is authored against the rendered skull
silhouette (0.168), not the rig's nominal head radius.

Growth stages 0–3 tend the rows with the `construct_chisel` and
`construct_kneel_chisel` clips. Ripe fields reap with `construct_reap` -- the
sickle clip the builders already use -- and the third hand bundles with
`resource_carry`. Actors are people, so the field's own scale never reaches
their world matrix; only its placement and row orientation do.
...
snapshot at the top of the frame — one over builder tasks, one over farms — and
submission is then a binary search on the resulting list. No component is looked
up per field per frame, and the live simulation is never read.
...
Anchors sit inside the crop rectangle shared by both faction models. Small
permanent work clearings around those anchors keep ripe wheat away from hands,
tools and the seated actor; these do not represent harvest progress. Faction
row orientation controls facing; stable entity hashes vary facing, placement
and phase. Each anchor resolves against the session terrain surface and the
...
Workers below seven projected pixels or beyond 160 world units are culled.
Below fifteen pixels, a field has at most one worker. Earlier visible fields
consume the budget; excess fields are skipped. The nap requires two visible
actors and is disabled by the one-worker LOD.
...
existing creature asset prewarm. Below twelve projected pixels a worker drops
to `CreatureLOD::Minimal`; that tier resolves a per-state snapshot mesh instead
of the requested clip, so it is kept to sizes where the pose cannot be read. The per-frame batch and the eligible-field and
real-task lists retain their vector capacity across frames.

The CPU sampling test reports `dense_submission_p95_us` for 100 field
submissions with the High cap, including field lookup, transforms, request
assembly and a recording submitter. This is a CPU microbenchmark, not a GPU frame-time measurement.
Use the dense Arena scene on target hardware to measure the full GPU cost;
the actor/primitive caps are structural budgets, not a promised millisecond cost.

## Lazy farmer

One renderer-wide slot permits at most one sequence, and no extra actors are
spawned -- the two workers already on the field play it. The beats are constants
in `Render::GL::Nap`, in seconds from the moment the slot is claimed:
...
| 7.2-9.0 | back to its normal loop                         | walks back to its own anchor             |
...
coworker does not skate. Worker 2 keeps working throughout.
...
```sh
QT_QPA_PLATFORM=offscreen build/bin/render_tests --gtest_filter='FarmActivityTest.*' --gtest_output=xml:/tmp/farm-activity.xml
...
DISPLAY=:0 build/bin/arena_app --batch --scenario farm_activity_single --duration 14 --scenario-distance 0.55 --clean-capture --capture-interval 3 --artifact-dir /tmp/farm-close
...
forty-second run — long enough for the full growth/ripen/reset/destroy cycle and
short enough to stay inside the arena watchdog on software GL.
`farm_activity_terrain` places both factions on a hill.
