...
public:
  ...
  [[nodiscard]] auto blob(std::uint32_t species_id) const noexcept -> const BpatBlob*;

  [[nodiscard]] auto has_species(std::uint32_t species_id) const noexcept -> bool;

  [[nodiscard]] auto find_clip(std::uint32_t species_id,
                               std::string_view name) const -> std::uint32_t;

  auto sample_palette(std::uint32_t species_id,
                      std::uint32_t clip_index,
                      std::uint32_t frame_in_clip,
                      std::span<QMatrix4x4> out) const -> std::uint32_t;

  auto sample_socket(std::uint32_t species_id,
                     std::uint32_t clip_index,
                     std::uint32_t frame_in_clip,
...
private:
  ...
  [[nodiscard]] auto species_slot(std::uint32_t id) const noexcept -> const BpatBlob*;
