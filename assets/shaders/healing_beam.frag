...
void main() {
  ...
  innerColor = mix(innerColor, u_heal_color, 0.5);
  outerColor = mix(outerColor, u_heal_color * 0.7, 0.5);

  float centerDist = v_edge_dist;
  ...
  float edgeShimmer = sin(v_beam_t * 50.0 - u_time * 10.0) * 0.5 + 0.5;
  edgeShimmer *= (1.0 - innerGlow) * outerGlow;
  color += sparkleColor * edgeShimmer * 0.2;

  float alpha = v_glow_intensity * outerGlow * u_alpha;

  float startFade = smoothstep(0.0, 0.1, v_beam_t);
  float endFade = smoothstep(u_progress, u_progress - 0.1, v_beam_t);
  alpha *= startFade * endFade;

  if (v_beam_t > 0.9 && u_progress > 0.95) {
    float impactGlow = (v_beam_t - 0.9) * 10.0;
