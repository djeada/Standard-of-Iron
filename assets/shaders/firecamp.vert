...
void main() {
  ...
  vec3 right_vec =
      right_len < 1e-4 ? vec3(1.0, 0.0, 0.0) : right_flat / right_len;
  ...
  float pulse = 0.94 +
                0.12 * sin(u_time * (u_flicker_speed * 0.62) + phase * 1.7) +
                0.05 * sin(u_time * (u_flicker_speed * 1.8) + phase * 2.8);
  float curl_noise =
      noise(vec2(centered_x * 1.6 + phase * 0.12,
  ...
  float width_scale = mix(
      width_base * pulse,
      width_base * (0.18 + 0.12 * gust_noise + 0.08 * curl_noise), tip_bias);
  float height_scale = clamp(radius * 0.24 * intensity_scale, 0.55, 1.08) *
                       (0.96 + 0.12 * pulse);

  float flicker_offset = (gust_noise - 0.5) * (u_flicker_amount * 0.7);
  float sway = sin(u_time * (u_flicker_speed * 1.08) + phase * 2.1 +
                   height_t * 5.3 + centered_x * 4.2);
  float lean =
      (sway * 0.55 + (curl_noise - 0.5) * 1.2 + (gust_noise - 0.5) * 1.1) *
      u_flicker_amount * radius * (0.12 + tip_bias * 1.15);
  float swirl =
      sin(u_time * (u_flicker_speed * 1.45) + phase * 3.1 + centered_x * 6.2);
  vec3 wobble_offset =
      horizontal_axis * ((sway * 0.32 + (gust_noise - 0.5) * 0.75) *
                         u_flicker_amount * radius * (0.08 + height_t * 0.42));
  vec3 lean_axis = normalize(mix(horizontal_axis, forward_vec,
                                 0.28 * sin(phase * 1.11 + u_time * 0.41)));

  vec3 local_offset =
      horizontal_axis * (centered_x * width_scale) +
  ...
  local_offset += forward_vec * (swirl * u_flicker_amount * radius *
                                 (0.03 + tip_bias * 0.22));

  float base_lift = radius * 0.02 + intensity * 0.04;
  float vertical_lift = (0.3 + 0.7 * curl_noise) * u_flicker_amount * radius *
                        (0.04 + height_t * 0.36);
