...
void main() {
  ...
  if (isHelmet) {
    ...
    float hingePins = cheekGuard * step(0.94, fract(v_bodyHeight * 28.0)) * 0.20;
    float guardRepoussé = cheekGuard * noise(uv * 12.0) * 0.08;  // Decorative embossing
    ...
    float neckSegments = step(0.85, fract(v_bodyHeight * 42.0)) * neckGuard * 0.15;
    ...
    float plumeSocket = plumeSocketHeight * smoothstep(0.05, 0.03, abs(v_worldPos.x)) * 0.28;
  ...
  float mudPattern = step(0.72, noise(v_worldPos.xz * 12.0 + v_worldPos.y * 3.0));
  ...
  float bloodPattern = step(0.88, noise(v_worldPos.xz * 15.0 + v_bodyHeight * 5.0));
