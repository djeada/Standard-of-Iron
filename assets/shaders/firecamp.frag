...
void main() {
  ...
  vec2 animated_uv = vec2(
      tex_coord.x, tex_coord.y + fract(u_time * 0.45 + flame_phase * 0.05));
  ...
  float flicker =
      mix(noise_low, noise_high, clamp(flame_height * 1.2, 0.0, 1.0));
  ...
  float edge_fade = smoothstep(0.0, 0.2, tex_coord.x) *
                    smoothstep(0.0, 0.2, 1.0 - tex_coord.x);
