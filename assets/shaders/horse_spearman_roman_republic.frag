...
void main() {
  ...
  if (isHorseHide) {
    ...
    if (isMuscleCuirass) {
      ...
      float muscleDef = smoothstep(0.40, 0.70, musclePattern) * 0.20;

      // Horizontal bands for ab definition
      float abBands = abs(sin(v_worldPos.y * 18.0)) * 0.12;
      ...
      float edges = smoothstep(0.80, 0.92, edgeNoise) * 0.15;

      // Decorative elements (medusa heads, eagles, wreaths)
      float decorPattern = step(0.88, fbm(v_worldPos.xz * 8.0)) * 0.18;
      ...
      albedo = albedo * (1.0 + decorPattern * 0.6);
      ...
      float cuirassBump = muscleDef * 3.0 + abBands * 2.5 + decorPattern * 2.0;
      N = perturbNormalWS(N, v_worldPos, cuirassBump, 0.60);
    ...
    if (isBowl) {
      ...
      float patina = patinaPattern * 0.28;
      vec3 patinaColor = vec3(0.35, 0.52, 0.42);

      // Battle damage (dents from blows)
      float dentPattern = fbm(v_worldPos.xz * 10.0);
      float dents = smoothstep(0.78, 0.88, dentPattern) * 0.18;
      ...
      float guardPatina = fbm(v_worldPos.xz * 7.0) * 0.35;
      vec3 patinaColor = vec3(0.35, 0.52, 0.42);
      ...
      roughness = 0.32 + guardPatina * 0.38;
      ...
      float topPatina = fbm(v_worldPos.xz * 5.0) * 0.20;
      vec3 patinaColor = vec3(0.35, 0.52, 0.42);
    ...
    // Bronze rim light
    float helmetRim = pow(1.0 - NdotV, 3.5) * 0.18;
