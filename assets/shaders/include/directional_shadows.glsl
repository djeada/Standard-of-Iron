...
layout(std140) uniform DirectionalShadows {
  ...
  vec4 u_shadow_params;       // enabled, cascade count, texel size, PCF radius
  vec4 u_shadow_camera_position;
  vec4 u_shadow_bias;         // depth bias, normal bias, cascade blend, unused
};

uniform sampler2DArray u_directional_shadow_map;

float sample_shadow_cascade(vec3 world_position,
                            vec3 normal,
                            int cascade) {
  ...
  float slope = 1.0 - max(dot(normalize(normal),
                              environment_primary_direction()), 0.0);
...
float directional_shadow_occlusion(vec3 world_position, vec3 normal) {
  ...
  int cascade = 0;
  if (camera_distance > u_shadow_split_distances.x) cascade = 1;
  if (camera_distance > u_shadow_split_distances.y) cascade = 2;
  if (camera_distance > u_shadow_split_distances.z) cascade = 3;
  ...
  if (cascade < cascade_count - 1) {
    ...
    if (blend > 0.0) {
      float next_shadow =
          sample_shadow_cascade(world_position, normal, cascade + 1);
...
vec3 apply_directional_shadow(vec3 lit_color,
                              vec3 world_position,
                              vec3 normal) {
  float amount = directional_shadow_occlusion(world_position, normal) *
                 environment_shadow_strength();
  return mix(lit_color,
             lit_color * environment_shadow_tint(),
