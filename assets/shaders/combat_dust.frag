...
void main() {
    // Create noise-like pattern for dust particles
    float noise1 = fract(sin(dot(v_texcoord * 10.0, vec2(12.9898, 78.233))) * 43758.5453);
    float noise2 = fract(sin(dot(v_texcoord * 15.0 + u_time * 0.1, vec2(93.9898, 67.345))) * 23421.631);
    float combined_noise = (noise1 + noise2) * 0.5;
    
    // Soft circular falloff per particle
    ...
    // Subtle light scattering effect
