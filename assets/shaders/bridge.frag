...
void main() {
  ...
  float grain = (soi_noise_3d41e6(uv_var * 18.0) - 0.5) * 0.08;

  vec3 stone_color = u_color * (1.0 + var_low + var_mid + grain);
  vec3 mortar_color = u_color * 0.72;

  float crack = smoothstep(0.02, 0.0, abs(soi_noise_3d41e6(uv * 10.0) - 0.5)) * 0.25;
  ...
  float grime = (1.0 - cavity) * 0.25 * (0.8 + 0.2 * soi_noise_3d41e6(uv * 7.0));
