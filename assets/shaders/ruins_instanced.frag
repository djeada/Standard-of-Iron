...
void main() {
  ...
  float stone_noise =
      0.5 + 0.5 * sin(v_local_pos.y * 12.0 + v_local_pos.x * 8.0 +
                      v_local_pos.z * 5.0);
  ...
  float fracture =
      smoothstep(0.88, 0.98,
                 abs(sin(v_local_pos.x * 18.0 - v_local_pos.z * 13.0 +
                         v_local_pos.y * 9.0)));
  vec3 stone_color = v_color * mix(0.82, 1.14, stone_noise);
  stone_color =
      mix(stone_color, stone_color * vec3(0.82, 0.84, 0.80), fracture * 0.35);
  ...
  float mortar_v =
      1.0 -
      smoothstep(0.04, 0.16, fract(v_local_pos.x * 1.9 + v_local_pos.z * 1.4));
