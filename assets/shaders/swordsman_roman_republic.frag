...
void main() {
  ...
  if (isHelmet) {
    ...
    float guardEdge = cheekGuard * smoothstep(0.36, 0.40, noise(uv * 16.0)) * 0.22;
    float hingeDetail = cheekGuard * step(0.92, fract(v_bodyHeight * 32.0)) * 0.25;
    ...
    float neckLamellae = step(0.82, fract(v_bodyHeight * 48.0)) * neckGuard * 0.18;
    float neckRivets = step(0.90, fract(v_worldPos.x * 22.0)) * neckGuard * 0.12;
    ...
    float plumeSocket = plumeSocketHeight * smoothstep(0.06, 0.04, abs(v_worldPos.x)) * 0.32;
  ...
  // HEAVY SEGMENTED ARMOR (lorica segmentata - iconic Roman plate armor)
    ...
    // ENHANCED: Plate segment definition (each band slightly different polish/color)
    float segmentVariation = noise(vec2(v_platePhase, 0.5) * 2.0) * 0.08;
    
    // ENHANCED: Overlapping plate shadows with depth (more realistic articulation)
    ...
    color += vec3(plateBrightness + plateSpecular + plateFresnel + 
                  skyReflect + brushedMetal + segmentVariation);
  ...
  float campaignWear = (1.0 - v_bodyHeight * 0.7) * (1.0 - v_polishLevel) * 0.12;
  float dustAccumulation = noise(v_worldPos.xz * 8.0) * campaignWear * 0.08;
  
  // Rain streaks (even polished armor shows weather)
  float rainStreaks = smoothstep(0.87, 0.93, noise(v_worldPos.xz * 2.5 + vec2(0.0, v_worldPos.y * 8.0))) * 
  ...
  float mudPattern = step(0.80, noise(v_worldPos.xz * 12.0 + v_worldPos.y * 3.0));
  ...
  float bloodPattern = step(0.90, noise(v_worldPos.xz * 15.0 + v_bodyHeight * 5.0));
  float bloodStain = bloodHeight * bloodPattern * (1.0 - v_polishLevel * 0.5) * 0.10;
