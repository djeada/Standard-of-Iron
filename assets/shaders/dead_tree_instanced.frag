...
void main() {
  ...
  float bark_noise = noise21(vec2(bark_axis + bark_angle * 0.8, bark_angle * 2.0 + 4.0));
  float bark_furrow =
  ...
  float bark_variation = clamp(0.18 + bark_furrow * 0.56 + bark_streak * 0.22, 0.0, 1.0);
  float bark_crack = smoothstep(0.74, 0.92, bark_furrow) * smoothstep(0.52, 0.80, bark_streak);
  ...
  vec2 end_delta = vec2((v_local_pos.y - end_center_y) / 0.19, (v_local_pos.z - end_center_z) / 0.20);
  float end_radius = length(end_delta);
  float end_noise = noise21(end_delta * 4.0 + vec2(abs_local_x * 3.0, bark_angle * 0.7 + 2.0));
  float end_rings = 0.5 + 0.5 * sin(end_radius * 18.0 - end_noise * 3.2);
  float end_angle = atan(end_delta.y, end_delta.x);
  float end_cracks = smoothstep(0.30, 0.92, end_radius) *
  ...
  float moss_t = clamp(moss_density * moss_noise * (1.0 - cut_face_mask) * 0.55, 0.0, 1.0);
  ...
  bark_color = mix(bark_color, bark_light, smoothstep(0.70, 0.96, bark_variation) * 0.30);
  bark_color *= 1.0 - bark_crack * 0.20;

  vec3 exposed_wood = mix(vec3(0.55, 0.43, 0.27), vec3(0.76, 0.63, 0.42), 0.30 + end_rings * 0.35 + end_noise * 0.20);
  ...
  material_color = mix(material_color, exposed_wood, end_mask * (1.0 - cut_face_mask) * 0.18);
