...
void main() {
  ...
  if (is_horse_hide) {
    ...
    // Swordsmen (elite) have higher armor_sheen, more likely muscle cuirass
    ...
    if (is_muscle_cuirass) {
      ...
      float muscle_def = smoothstep(0.40, 0.70, muscle_pattern) *
      ...
      float patina = patina_pattern * 0.22;       // Elite: less patina
      ...
      float dents =
          smoothstep(0.78, 0.88, dent_noise) * 0.12; // Elite: less damage
      ...
      albedo =
          albedo * (1.0 + decor_pattern * 0.7); // Elite: brighter decoration
    ...
    float armor_rim = pow(1.0 - NdotV, 4.0) * 0.16; // Elite: brighter rim
    ...
    if (is_bowl) {
      ...
      float patina = patina_pattern * 0.25; // Elite: less patina
      vec3 patina_color = vec3(0.35, 0.52, 0.42);

      // Battle damage (dents from blows)
      float dent_pattern = fbm(v_worldPos.xz * 10.0);
      float dents =
          smoothstep(0.80, 0.90, dent_pattern) * 0.15; // Elite: less damage
      ...
      roughness = 0.30 + guard_patina * 0.35; // Elite: smoother
      ...
      float top_patina = fbm(v_worldPos.xz * 5.0) * 0.18; // Elite: even less
      vec3 patina_color = vec3(0.35, 0.52, 0.42);
      ...
      roughness = 0.25 + top_patina * 0.28; // Elite: smoother
    }
    ...
    // Bronze rim light
    float helmet_rim = pow(1.0 - NdotV, 3.5) * 0.20; // Elite: brighter rim
