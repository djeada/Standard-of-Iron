...
// Pseudo-random hash for variation
float hash(float n) {
...
void main() {
    ...
    // Calculate how far down the banner this vertex is (0 = top, 1 = bottom)
    // Assuming banner is centered at origin with height of 1
    float verticalPos = clamp(0.5 - pos.y, 0.0, 1.0);
    
    // Calculate horizontal position for wave variation
    float horizontalPos = pos.x + 0.5;
    
    // Wind wave - increases toward bottom of banner (cloth physics simulation)
    ...
    // Secondary ripple for organic movement
    ...
    float xOffset = sin(swayPhase + verticalPos * 0.5) * u_windStrength * verticalPos * 0.03;
    
    // Apply wave deformation
    pos.z += zOffset + ripple;
    pos.x += xOffset;
    
    // Slight droop toward bottom
