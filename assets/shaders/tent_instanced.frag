...
void main() {
  ...
  float center_pole = (1.0 - smoothstep(0.035, 0.072, max(abs(v_local_pos.x),
                                                          abs(v_local_pos.z)))) *
  ...
  float wood_mask = clamp(max(max(front_posts, front_crossbar),
                              max(center_pole, awning_posts)), 0.0, 1.0);
  ...
  float ridge_seam = band(v_local_pos.x, 0.0, 0.018, 0.020) *
                     smoothstep(0.44, 0.72, v_local_pos.y);
  ...
  float sun_bleach = smoothstep(0.50, 0.88, v_local_pos.y) *
                     smoothstep(0.18, 0.76, v_local_normal.y);
  ...
  vec3 iron = mix(vec3(0.10, 0.11, 0.12), vec3(0.28, 0.25, 0.21),
                  hash12(v_world_pos.xz * 7.0));
  ...
  float specular = pow(max(dot(N, H), 0.0), 58.0) *
                   mix(0.025, 0.20, iron_mask);
