...
float horseHidePattern(vec2 p) {
  float hairGrain = noise(p * 80.0) * 0.12;
  float muscleRipple = sin(p.x * 25.0) * cos(p.y * 30.0) * 0.04;
  float hideSpots = smoothstep(0.45, 0.55, noise(p * 8.0)) * 0.08;

  return hairGrain + muscleRipple + hideSpots;
}
...
void main() {
  ...
  bool isHorseHide = (avgColor < 0.40 && colorHue < 0.12 && v_worldPos.y < 0.8);

  if (isHorseHide) {
    float hideTexture = horseHidePattern(v_worldPos.xz);
    float hairSheen = noise(uv * 15.0) * 0.08;

    float viewAngle = abs(dot(normal, normalize(vec3(0.0, 1.0, 0.5))));
    float hideSpecular = pow(viewAngle, 6.0) * 0.15;

    color += vec3(hideTexture + hideSpecular + hairSheen);
    color *= 1.0 + noise(uv * 12.0) * 0.10 - 0.05;
