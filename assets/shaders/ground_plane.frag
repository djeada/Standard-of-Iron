...
void main() {
  ...
  vec2 warp_noise = vec2(fbm(base_uv * 0.22), fbm((base_uv + vec2(13.7, -6.1)) * 0.22));
  ...
  float raised_shelf = smoothstep(0.54, 0.86, landform * 0.72 + landform2 * 0.28);
  ...
  float wind_scour = smoothstep(0.45, 0.82, detail + rise * 0.55 + slope * 0.30);
  ...
  float gravel_mask = smoothstep(0.52,
                                 0.86,
  ...
  col *= 1.0 +
         ((turf_fine - 0.5) * 0.11 + (turf_micro - 0.5) * 0.06) * turf_weight;
  col = mix(col,
            col * vec3(0.93, 1.06, 0.89),
            smoothstep(0.60, 0.92, turf_fine * 0.72 + turf_micro * 0.28) *
                turf_weight * 0.40);
  col *= u_tint;
