...
// Dash pattern calculation
float dashPattern(float distance, float dashLen, float gapLen, float animOffset) {
...
void main() {
    ...
    // Apply ink texture for handdrawn look
    ...
    if (u_use_dash_pattern) {
        ...
        float dashMask = dashPattern(v_distance, u_dash_length, u_gap_length, animOffset);
    ...
    // Apply edge feathering for soft antialiased appearance
    if (u_use_feather) {
        // v_uv.y represents cross-stroke position [-1, 1]
