...
uniform int u_badge_style; // 0=standard, 1=seal, 2=banner, 3=shield, 4=medallion
uniform bool u_show_shadow;
uniform bool u_animate;

out vec4 fragColor;

// Badge shape SDF functions
...
float sdShield(vec2 p) {
    ...
    // Top half is rounded rectangle
    if (p.y < 0.0) {
        float d = sdBox(vec2(q.x, 0.0), vec2(0.4, 0.3));
        return d;
    }
    
    // Bottom half curves to point
    float r = 0.5;
...
float sdBanner(vec2 p) {
    // Banner shape: rectangle with V-cut bottom
    vec2 q = abs(p);
...
float sdMedallion(vec2 p) {
    // Medallion: circle with decorative edge
    float r = 0.42;
    float d = length(p) - r;
...
float sdSeal(vec2 p) {
    // Wax seal shape: irregular circle
    float r = 0.4;
    float angle = atan(p.y, p.x);
    
    // Create wavy irregular edge like wax
...
float sdStandard(vec2 p) {
    // Military standard: pole with flag
    vec2 q = abs(p);
...
// Get badge shape SDF based on style
float getBadgeSDF(vec2 p, int style) {
    if (style == 0) return sdStandard(p);
    if (style == 1) return sdSeal(p);
    if (style == 2) return sdBanner(p);
    if (style == 3) return sdShield(p);
    if (style == 4) return sdMedallion(p);
    return sdCircle(p, 0.4); // fallback
}

// Emblem decoration pattern
float getEmblemPattern(vec2 p, int style) {
    if (style == 1) {
        // Seal: laurel wreath pattern
        float angle = atan(p.y, p.x);
        float r = length(p);
    ...
    if (style == 3 || style == 4) {
        // Shield/Medallion: cross or star pattern
        vec2 q = abs(p);
...
void main() {
    ...
    // Add subtle lighting/bevel effect
