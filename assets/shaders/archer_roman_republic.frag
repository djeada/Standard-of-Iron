...
void main() {
  ...
  if (isHelmet) {
    ...
    float plumeSocket =
        plumeSocketHeight * smoothstep(0.04, 0.02, abs(v_worldPos.x)) * 0.25;
    ...
    color +=
        vec3(cheekGuard + guardEdge + neckGuard + neckSegments + plumeSocket);
  ...
  // pattern)
    ...
    float oxidation =
        smoothstep(0.45, 0.75, rustSeed) * v_chainmailPhase * 0.32;
