...
inline auto clamp_vec(const QVector3D& c, float min_val, float max_val) -> QVector3D {
  return {clamp_f(c.x(), min_val, max_val),
          clamp_f(c.y(), min_val, max_val),
...
inline auto
lerp(const QVector3D& a, const QVector3D& b, float t) noexcept -> QVector3D {
  return a * (1.0F - t) + b * t;
}
...
inline auto
nlerp(const QVector3D& a, const QVector3D& b, float t) noexcept -> QVector3D {
