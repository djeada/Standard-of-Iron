...
inline void set_affine_columns(QMatrix4x4& m,
                               float xx,
                               float xy,
                               float xz,
                               float yx,
...
auto build_oriented_cylinder_matrix(const QVector3D& a,
                                    const QVector3D& b,
  ...
  if (len_sq <= k_eps_sq) {
    ...
    set_affine_columns(m,
                       right.x() * radius_right,
                       right.y() * radius_right,
                       right.z() * radius_right,
                       up.x(),
                       up.y(),
                       up.z(),
                       forward.x() * radius_forward,
                       forward.y() * radius_forward,
                       forward.z() * radius_forward,
                       centre.x(),
                       centre.y(),
                       centre.z());
    return m;
...
auto build_oriented_basis_matrix(const Render::Math::BoneFrame& frame,
  ...
  QMatrix4x4 m;
  set_affine_columns(m,
                     frame.right.x() * scale.x(),
                     frame.right.y() * scale.x(),
                     frame.right.z() * scale.x(),
                     frame.up.x() * scale.y(),
                     frame.up.y() * scale.y(),
                     frame.up.z() * scale.y(),
                     frame.forward.x() * scale.z(),
                     frame.forward.y() * scale.z(),
                     frame.forward.z() * scale.z(),
                     frame.origin.x(),
                     frame.origin.y(),
                     frame.origin.z());
  return m;
...
auto oriented_cylinder(const QVector3D& a,
                       const QVector3D& b,
                       const QVector3D& right_reference,
                       float radius_right,
                       float radius_forward) -> QMatrix4x4 {
  return build_oriented_cylinder_matrix(
      a, b, right_reference, radius_right, radius_forward);
}

auto oriented_cylinder(const QMatrix4x4& parent,
                       const QVector3D& a,
                       const QVector3D& b,
                       const QVector3D& right_reference,
                       float radius_right,
