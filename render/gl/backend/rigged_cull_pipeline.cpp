...
constexpr std::size_t k_floats_per_instance = 32;
...
auto RiggedCullPipeline::upload_instances(const RiggedCreatureCmd* const* cmds,
  ...
  m_palette_scratch.assign(count * bone_count * 16, 0.0F);
  ...
  for (std::size_t k = 0; k < count; ++k) {
    ...
    float* palette_dst = m_palette_scratch.data() + k * bone_count * 16;
    for (std::size_t b = 0; b < bone_count; ++b) {
      std::memcpy(
          palette_dst + b * 16, cmd.bone_palette[b].constData(), sizeof(float) * 16);
    }
...
auto RiggedCullPipeline::draw(const RiggedCreatureCmd* const* cmds,
  ...
  RiggedMesh* mesh = cmds[0]->mesh;
  if (mesh == nullptr || cmds[0]->texture != nullptr || cmds[0]->bone_count == 0) {
    return false;
  }
  std::size_t const bone_count = cmds[0]->bone_count;
  for (std::size_t k = 0; k < count; ++k) {
    if (cmds[k] == nullptr || cmds[k]->mesh != mesh || cmds[k]->texture != nullptr ||
        cmds[k]->bone_palette == nullptr || cmds[k]->bone_count != bone_count) {
      return false;
    }
  }
  ...
  bool const cull_backfaces = cull_enabled == GL_TRUE && cull_mode == GL_BACK;
  ...
  cull->set_uniform(cull->uniform_handle("u_front_face_ccw"),
                    front_face == GL_CCW ? 1 : 0);
  ...
  m_draw_shader->use();
  m_draw_shader->set_uniform(m_draw_shader->uniform_handle("u_view_proj"), view_proj);
  glUniform1ui(m_draw_shader->uniform_handle("u_vertex_count"),
               static_cast<GLuint>(mesh->vertex_count()));
  glUniform1ui(m_draw_shader->uniform_handle("u_bone_count"),
               static_cast<GLuint>(bone_count));
  m_draw_shader->set_uniform(m_draw_shader->uniform_handle("u_camera_position"),
                             camera_position);
  auto const role_tbo = m_draw_shader->optional_uniform_handle("u_role_color_tbo");
  if (role_tbo != Shader::InvalidUniform) {
    m_draw_shader->set_uniform(role_tbo, 0);
  }
