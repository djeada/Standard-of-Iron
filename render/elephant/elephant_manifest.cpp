...
auto build_elephant_whole_nodes()
  ...
  body.rings = {
      ...
      {bl * 0.32F,
       bh * 0.06F,
       bw * 0.86F * k_elephant_torso_width_scale,
       bh * 0.40F * k_elephant_torso_height_scale,
       bh * 0.36F * k_elephant_torso_height_scale},
      {bl * 0.50F,
       bh * 0.10F,
       bw * 0.74F * k_elephant_torso_width_scale,
       bh * 0.38F * k_elephant_torso_height_scale,
       bh * 0.28F * k_elephant_torso_height_scale},
      {bl * 0.62F,
       bh * 0.14F,
       bw * 0.50F * k_elephant_torso_width_scale,
       bh * 0.24F * k_elephant_torso_height_scale,
       bh * 0.14F * k_elephant_torso_height_scale},
      {bl * 0.70F,
       bh * 0.13F,
       bw * 0.28F * k_elephant_torso_width_scale,
       bh * 0.10F * k_elephant_torso_height_scale,
       bh * 0.04F * k_elephant_torso_height_scale},
  };
  nodes.push_back({"elephant.body",
                   static_cast<BoneIndex>(ElephantBone::Body),
                   k_role_skin,
                   k_lod_all,
                   0,
                   body});
  ...
  auto add_leg = [&](std::string_view thigh_name,
    ...
    float const hoof_height = dims.foot_radius * k_elephant_visual_scale * 0.86F *
    ...
    float const bend_z =
        (front ? 1.0F : -1.0F) * dims.body_length * k_elephant_visual_scale * 0.16F;
    float const ankle_z =
        z + (front ? 1.0F : -1.0F) * dims.body_length * k_elephant_visual_scale * 0.06F;
    ...
    float const thigh_trim_y =
        (knee_local.y() - shoulder_local.y()) * k_elephant_whole_thigh_upper_trim;
    ...
    nodes.push_back({thigh_name, shoulder_bone, k_role_skin, k_lod_all, 0, thigh});
  ...
  add_leg("elephant.leg.fl.thigh",
          "elephant.leg.fl.calf",
          "elephant.leg.fl.hoof",
          bw * 0.40F,
          bl * 0.42F,
          static_cast<BoneIndex>(ElephantBone::ShoulderFL),
          static_cast<BoneIndex>(ElephantBone::KneeFL),
          static_cast<BoneIndex>(ElephantBone::FootFL),
          true);
  add_leg("elephant.leg.fr.thigh",
          "elephant.leg.fr.calf",
          "elephant.leg.fr.hoof",
          -bw * 0.40F,
          bl * 0.42F,
          static_cast<BoneIndex>(ElephantBone::ShoulderFR),
          static_cast<BoneIndex>(ElephantBone::KneeFR),
          static_cast<BoneIndex>(ElephantBone::FootFR),
          true);
  add_leg("elephant.leg.bl.thigh",
          "elephant.leg.bl.calf",
          "elephant.leg.bl.hoof",
          bw * 0.38F,
          -bl * 0.44F,
          static_cast<BoneIndex>(ElephantBone::ShoulderBL),
          static_cast<BoneIndex>(ElephantBone::KneeBL),
          static_cast<BoneIndex>(ElephantBone::FootBL),
          false);
  add_leg("elephant.leg.br.thigh",
          "elephant.leg.br.calf",
          "elephant.leg.br.hoof",
          -bw * 0.38F,
          -bl * 0.44F,
          static_cast<BoneIndex>(ElephantBone::ShoulderBR),
          static_cast<BoneIndex>(ElephantBone::KneeBR),
          static_cast<BoneIndex>(ElephantBone::FootBR),
          false);
  ...
  auto add_tusk = [&](float side) {
    ...
    tusk.tip =
        QVector3D(side * hw * 0.42F,
                  -bh * 0.34F,
                  bl * 1.10F + dims.tusk_length * k_elephant_visual_scale * 0.24F);
  ...
  auto make_ear = [&](float side) {
    ...
    ear.thickness =
        dims.ear_thickness * k_elephant_visual_scale * 1.15F * k_elephant_ear_scale;
    nodes.push_back({"elephant.ear",
                     static_cast<BoneIndex>(ElephantBone::Head),
                     k_role_skin,
                     k_lod_all,
                     0,
...
auto build_elephant_leg_span_mesh(std::span<const ElephantLegRingProfile> rings)
  ...
  for (ElephantLegRingProfile const& ring : rings) {
    ...
    append_vertex(ring.x_shift - ring.x_inner * 0.60F,
                  ring.y,
                  ring.z_shift - ring.z_back * 0.84F,
                  0.56F,
                  ring.y + 0.5F);
    append_vertex(
...
auto build_elephant_full_leg_overlays()
  ...
  auto add_leg = [&](std::string_view thigh_name,
    ...
    thigh.params.radius =
        dims.leg_radius * (front ? 1.95F : 1.90F) * k_elephant_thigh_thickness_scale;
    thigh.params.depth_radius =
        dims.leg_radius * (front ? 2.25F : 2.10F) * k_elephant_thigh_thickness_scale;
    thigh.custom_mesh =
  ...
  add_leg("elephant.leg.fl.thigh",
          ...
          static_cast<Render::Creature::BoneIndex>(ElephantBone::FootFL),
          true);
  add_leg("elephant.leg.fr.thigh",
          "elephant.leg.fr.calf",
          "elephant.leg.fr.hoof",
          ...
          static_cast<Render::Creature::BoneIndex>(ElephantBone::FootFR),
          true);
  add_leg("elephant.leg.bl.thigh",
          "elephant.leg.bl.calf",
          "elephant.leg.bl.hoof",
          ...
          static_cast<Render::Creature::BoneIndex>(ElephantBone::FootBL),
          false);
  add_leg("elephant.leg.br.thigh",
          "elephant.leg.br.calf",
          "elephant.leg.br.hoof",
          ...
          static_cast<Render::Creature::BoneIndex>(ElephantBone::FootBR),
          false);
  return out;
}
...
auto elephant_topology_ref() noexcept -> const Render::Creature::SkeletonTopology& {
  static const auto topology = elephant_topology_storage().topology();
  return topology;
}

void bake_elephant_manifest_clip_palettes(std::size_t clip_index,
