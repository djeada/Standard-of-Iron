...
inline auto
rand_between(uint32_t seed, uint32_t salt, float min_val, float max_val) -> float {
...
auto make_elephant_dimensions(uint32_t seed) -> ElephantDimensions {
  ...
  d.body_length =
      rand_between(seed, k_salt_body_length, k_body_length_min, k_body_length_max);
  d.body_width =
      rand_between(seed, k_salt_body_width, k_body_width_min, k_body_width_max);
  d.body_height =
      rand_between(seed, k_salt_body_height, k_body_height_min, k_body_height_max);

  d.neck_length =
      rand_between(seed, k_salt_neck_length, k_neck_length_min, k_neck_length_max);
  d.neck_width =
      rand_between(seed, k_salt_neck_width, k_neck_width_min, k_neck_width_max);

  d.head_length =
      rand_between(seed, k_salt_head_length, k_head_length_min, k_head_length_max);
  d.head_width =
      rand_between(seed, k_salt_head_width, k_head_width_min, k_head_width_max);
  d.head_height =
      rand_between(seed, k_salt_head_height, k_head_height_min, k_head_height_max);

  d.trunk_length =
      rand_between(seed, k_salt_trunk_length, k_trunk_length_min, k_trunk_length_max);
  d.trunk_base_radius = rand_between(
      seed, k_salt_trunk_base_radius, k_trunk_base_radius_min, k_trunk_base_radius_max);
  d.trunk_tip_radius = rand_between(
      seed, k_salt_trunk_tip_radius, k_trunk_tip_radius_min, k_trunk_tip_radius_max);

  d.ear_width = rand_between(seed, k_salt_ear_width, k_ear_width_min, k_ear_width_max);
  d.ear_height =
      rand_between(seed, k_salt_ear_height, k_ear_height_min, k_ear_height_max);
  d.ear_thickness = rand_between(
      seed, k_salt_ear_thickness, k_ear_thickness_min, k_ear_thickness_max);
  ...
  d.foot_radius =
      rand_between(seed, k_salt_foot_radius, k_foot_radius_min, k_foot_radius_max);

  d.foot_radius *= (1.0F / 1.2F);

  d.tail_length =
      rand_between(seed, k_salt_tail_length, k_tail_length_min, k_tail_length_max);

  d.tusk_length =
      rand_between(seed, k_salt_tusk_length, k_tusk_length_min, k_tusk_length_max);
  d.tusk_radius =
      rand_between(seed, k_salt_tusk_radius, k_tusk_radius_min, k_tusk_radius_max);

  d.howdah_width =
      rand_between(seed, k_salt_howdah_width, k_howdah_width_min, k_howdah_width_max);
  d.howdah_length = rand_between(
      seed, k_salt_howdah_length, k_howdah_length_min, k_howdah_length_max);
  d.howdah_height = rand_between(
      seed, k_salt_howdah_height, k_howdah_height_min, k_howdah_height_max);

  d.idle_bob_amplitude = rand_between(
      seed, k_salt_idle_bob, k_idle_bob_amplitude_min, k_idle_bob_amplitude_max);
  d.move_bob_amplitude = rand_between(
      seed, k_salt_move_bob, k_move_bob_amplitude_min, k_move_bob_amplitude_max);
...
auto make_elephant_variant(uint32_t seed,
  ...
  v.skin_color = QVector3D(k_skin_base_r * skin_variation,
                           k_skin_base_g * skin_variation,
  ...
  float const shadow_t = rand_between(seed, k_salt_shadow, 0.0F, k_shadow_blend);
...
auto make_elephant_profile(uint32_t seed,
  ...
  profile.gait.front_leg_phase = rand_between(
      seed, k_salt_front_leg_phase, k_front_leg_phase_min, k_front_leg_phase_max);
  ...
  profile.gait.stride_swing =
      rand_between(seed, k_salt_stride_swing, k_stride_swing_min, k_stride_swing_max);
  profile.gait.stride_lift =
      rand_between(seed, k_salt_stride_lift, k_stride_lift_min, k_stride_lift_max);
