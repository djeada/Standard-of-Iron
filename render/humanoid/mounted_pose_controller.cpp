...
void MountedPoseController::mountOnHorse(const HorseMountFrame &mount) {
...
void MountedPoseController::ridingIdle(const HorseMountFrame &mount) {
  mountOnHorse(mount);

  // Hands resting on thighs or holding reins loosely
  using HP = HumanProportions;
  QVector3D const left_hand_rest(m_pose.pelvisPos.x() - 0.12F,
                                 m_pose.pelvisPos.y() - 0.08F,
                                 m_pose.pelvisPos.z() + 0.15F);
  QVector3D const right_hand_rest(m_pose.pelvisPos.x() + 0.12F,
                                  m_pose.pelvisPos.y() - 0.08F,
                                  m_pose.pelvisPos.z() + 0.15F);
...
void MountedPoseController::ridingLeaning(const HorseMountFrame &mount,
                                          float forward_lean,
                                          float side_lean) {
  ...
  // Lean torso based on input
  QVector3D const lean_offset =
      mount.seat_forward * (forward_lean * 0.15F) +
  ...
  // Adjust hands for leaning
  QVector3D const left_hand =
      mount.rein_attach_left + QVector3D(0.0F, -0.10F, 0.0F) + lean_offset * 0.5F;
  QVector3D const right_hand =
      mount.rein_attach_right + QVector3D(0.0F, -0.10F, 0.0F) + lean_offset * 0.5F;
...
void MountedPoseController::ridingCharging(const HorseMountFrame &mount,
                                           float intensity) {
  intensity = std::clamp(intensity, 0.0F, 1.0F);

  mountOnHorse(mount);

  // Lean forward aggressively during charge
  ...
  // Grip reins tightly
  holdReins(mount, 0.2F, 0.2F);
}

void MountedPoseController::ridingReining(const HorseMountFrame &mount,
                                          float left_tension,
                                          float right_tension) {
  ...
  // Pull back on reins with tension
  QVector3D const left_rein_pos =
      mount.rein_attach_left + mount.seat_forward * (-0.15F * left_tension) +
      QVector3D(0.0F, -0.05F - 0.08F * left_tension, 0.0F);
  QVector3D const right_rein_pos =
      mount.rein_attach_right + mount.seat_forward * (-0.15F * right_tension) +
      QVector3D(0.0F, -0.05F - 0.08F * right_tension, 0.0F);
  ...
  // Lean back slightly when reining
...
void MountedPoseController::ridingMeleeStrike(const HorseMountFrame &mount,
                                              float attack_phase) {
  using HP = HumanProportions;
  ...
  // Strike animation phases
  QVector3D const rest_pos = mount.seat_position +
                             mount.seat_right * 0.15F +
                             QVector3D(0.0F, 0.05F, 0.0F);
  QVector3D const raised_pos = mount.seat_position +
                               mount.seat_right * 0.20F +
                               QVector3D(0.0F, 0.50F, 0.0F);
  QVector3D const strike_pos = mount.seat_position +
                               mount.seat_forward * 0.40F +
                               mount.seat_right * 0.25F +
                               QVector3D(0.0F, 0.05F, 0.0F);

  QVector3D hand_r_target;
  QVector3D hand_l_target = mount.rein_attach_left + QVector3D(0.0F, -0.12F, 0.0F);
...
void MountedPoseController::ridingSpearThrust(const HorseMountFrame &mount,
                                              float attack_phase) {
  attack_phase = std::clamp(attack_phase, 0.0F, 1.0F);

  mountOnHorse(mount);

  // Spear thrust from mounted position
  QVector3D const guard_pos = mount.seat_position +
                              mount.seat_right * 0.15F +
                              mount.seat_forward * 0.10F +
                              QVector3D(0.0F, 0.10F, 0.0F);
  QVector3D const thrust_pos = mount.seat_position +
                               mount.seat_forward * 0.85F +
                               mount.seat_right * 0.10F +
...
void MountedPoseController::ridingBowShot(const HorseMountFrame &mount,
                                          float draw_phase) {
  using HP = HumanProportions;
  ...
  // Bow shooting from horseback
  QVector3D const bow_hold_pos = mount.seat_position +
                                 mount.seat_forward * 0.25F +
                                 mount.seat_right * (-0.08F) +
                                 QVector3D(0.0F, 0.25F, 0.0F);
  QVector3D const draw_start_pos = bow_hold_pos +
                                   mount.seat_right * 0.08F +
                                   QVector3D(0.0F, -0.05F, 0.0F);
  QVector3D const draw_end_pos = mount.seat_position +
                                 mount.seat_right * 0.12F +
...
void MountedPoseController::ridingShieldDefense(const HorseMountFrame &mount,
                                                bool raised) {
  mountOnHorse(mount);

  QVector3D shield_pos;
  if (raised) {
    // Raised defensive position
    shield_pos = mount.seat_position +
                 mount.seat_right * (-0.18F) +
                 mount.seat_forward * 0.15F +
                 QVector3D(0.0F, 0.40F, 0.0F);
  } else {
    // Lowered/resting position
    shield_pos = mount.seat_position +
                 mount.seat_right * (-0.15F) +
                 QVector3D(0.0F, 0.08F, 0.0F);
  }

  QVector3D const rein_pos = mount.rein_attach_right + QVector3D(0.0F, -0.10F, 0.0F);
...
void MountedPoseController::holdReins(const HorseMountFrame &mount,
                                      float left_slack, float right_slack) {
  left_slack = std::clamp(left_slack, 0.0F, 1.0F);
  right_slack = std::clamp(right_slack, 0.0F, 1.0F);

  // Position hands based on rein slack
  QVector3D const left_rein_pos =
      mount.rein_attach_left +
      QVector3D(0.0F, -0.08F - left_slack * 0.08F, 0.0F) +
      mount.seat_forward * (left_slack * -0.05F);
  QVector3D const right_rein_pos =
      mount.rein_attach_right +
      QVector3D(0.0F, -0.08F - right_slack * 0.08F, 0.0F) +
      mount.seat_forward * (right_slack * -0.05F);
...
void MountedPoseController::holdSpearMounted(const HorseMountFrame &mount,
  ...
  case SpearGrip::OVERHAND:
    ...
    hand_r_target = mount.seat_position +
                    mount.seat_right * 0.12F +
                    QVector3D(0.0F, 0.55F, 0.0F);
    hand_l_target = mount.rein_attach_left + QVector3D(0.0F, -0.10F, 0.0F);
    break;

  case SpearGrip::COUCHED:
    // Lance-style couched under arm
    hand_r_target = mount.seat_position +
                    mount.seat_right * 0.08F +
                    mount.seat_forward * (-0.15F) +
                    QVector3D(0.0F, 0.08F, 0.0F);
...
void MountedPoseController::holdBowMounted(const HorseMountFrame &mount) {
  mountOnHorse(mount);

  // Hold bow ready position while mounted
  QVector3D const bow_hold_pos = mount.seat_position +
                                 mount.seat_forward * 0.20F +
                                 mount.seat_right * (-0.08F) +
                                 QVector3D(0.0F, 0.22F, 0.0F);
  QVector3D const arrow_nock_pos = bow_hold_pos +
                                   mount.seat_right * 0.10F +
...
void MountedPoseController::attachFeetToStirrups(const HorseMountFrame &mount) {
  // Place feet in stirrups
  m_pose.footL = mount.stirrup_bottom_left;
  m_pose.foot_r = mount.stirrup_bottom_right;
}

void MountedPoseController::positionPelvisOnSaddle(
    const HorseMountFrame &mount) {
  // Position pelvis at seat position
  m_pose.pelvisPos = mount.seat_position;

  // Lift upper body proportionally
  float const saddle_y = mount.seat_position.y();
  liftBodyToMountHeight(saddle_y);
}

void MountedPoseController::liftBodyToMountHeight(float saddle_height) {
  using HP = HumanProportions;

  // Calculate height adjustment
  float const offset_y = saddle_height - HP::WAIST_Y;

  // Lift shoulders, neck, and head
  m_pose.shoulderL.setY(m_pose.shoulderL.y() + offset_y);
  m_pose.shoulderR.setY(m_pose.shoulderR.y() + offset_y);
  m_pose.neck_base.setY(m_pose.neck_base.y() + offset_y);
  m_pose.headPos.setY(m_pose.headPos.y() + offset_y);
}

void MountedPoseController::calculateRidingKnees(const HorseMountFrame &mount) {
  // Calculate knee positions using IK
  QVector3D const hip_left = m_pose.pelvisPos + QVector3D(-0.08F, -0.02F, 0.0F);
  QVector3D const hip_right = m_pose.pelvisPos + QVector3D(0.08F, -0.02F, 0.0F);
...
auto MountedPoseController::solveKneeIK(bool is_left, const QVector3D &hip,
  ...
  // Bend preference for mounted riding (knees out to sides)
  QVector3D bend_pref =
      is_left ? QVector3D(-0.85F, 0.0F, 0.45F) : QVector3D(0.85F, 0.0F, 0.45F);
  bend_pref.normalize();
...
auto MountedPoseController::computeOutwardDir(bool is_left) const
    -> QVector3D {
