...
public:
  ...
  [[nodiscard]] auto
  sway_clips() const noexcept -> const std::vector<Render::Animation::Clip<float>>& {
    return m_sway_clips;
  }
  ...
  [[nodiscard]] auto
  jitter_clips() const noexcept -> const std::vector<Render::Animation::Clip<float>>& {
...
public:
  ...
  void
  tick(float dt, const Render::GL::AnimationInputs& inputs, bool dead_flag = false);
  ...
  [[nodiscard]] auto state_machine() const noexcept -> const HumanoidStateMachine& {
    return m_state;
  }
