      ...
      if (entry.state == Engine::Core::DeathSequenceState::Dying) {
        ...
        casualty_anim.death_progress =
            (entry.state_duration > 0.0F)
                ? std::clamp(entry.state_time / entry.state_duration, 0.0F,
                             1.0F)
                : 1.0F;
      } else {
