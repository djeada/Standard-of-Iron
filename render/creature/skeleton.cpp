...
auto orthonormalise_x(const QVector3D& right_hint,
...
auto frame_matrix(const Render::GL::AttachmentFrame& frame) noexcept -> QMatrix4x4 {
  QMatrix4x4 m;
...
auto make_bone_basis(const QVector3D& head,
                     const QVector3D& tail,
                     const QVector3D& right_hint) noexcept -> QMatrix4x4 {
  QVector3D const y_axis = normalised_or(tail - head, QVector3D(0.0F, 1.0F, 0.0F));
