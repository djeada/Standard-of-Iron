...
auto humanoid_compute_bones(const void *pose, std::span<QMatrix4x4> out)
    -> std::uint32_t {
  return Render::Humanoid::compute_bone_palette(
      *static_cast<const Render::GL::HumanoidPose *>(pose), out);
}

auto horse_compute_bones(const void *pose, std::span<QMatrix4x4> out)
    -> std::uint32_t {
  return Render::Horse::compute_horse_bone_palette(
      *static_cast<const Render::Horse::HorseSpecPose *>(pose), out);
}

auto elephant_compute_bones(const void *pose, std::span<QMatrix4x4> out)
    -> std::uint32_t {
...
auto humanoid_fill_roles(const void *variant, QVector3D *out,
  ...
  for (std::size_t i = 0; i < n; ++i) out[i] = filled[i];
...
CreatureAssetRegistry::CreatureAssetRegistry() {
  ...
  m_humanoid.role_count = static_cast<std::uint8_t>(Render::Humanoid::kHumanoidRoleCount);
  m_humanoid.max_bones = static_cast<std::uint8_t>(Render::Humanoid::kBoneCount);
  ...
  m_elephant.max_bones = static_cast<std::uint8_t>(Render::Elephant::kElephantBoneCount);
