...
auto locomotion_intensity(bool is_moving, bool is_running, const Gait &gait,
  ...
  float const stride_lift =
      std::clamp(gait.stride_lift * config.stride_lift_intensity_scale, 0.0F,
                 1.0F);
  ...
  return std::clamp(config.base_intensity +
                        ...
                        stride_lift * config.lift_intensity_scale +
                        running_bonus,
                    0.0F, 1.0F);
}
...
auto default_foot_position(const Dimensions &dims, LegId leg,
  ...
  QVector3D const hip = barrel_center +
                        QVector3D(leg_lateral_sign(leg) * dims.body_width *
                                      lateral_scale,
                                  -dims.body_height * vertical_scale,
                                  leg_forward_sign(leg) * dims.body_length *
                                      fore_aft_scale);
...
auto swing_target(const Dimensions &dims, LegId leg,
                  ...
                  float lateral_scale, float vertical_scale,
                  float fore_aft_scale) noexcept -> QVector3D {
  QVector3D target = default_foot_position(dims, leg, barrel_center,
                                           lateral_scale, vertical_scale,
                                           fore_aft_scale);
...
auto body_sway(bool is_moving, float phase, float time, float move_intensity,
  ...
  float const amplitude = config.base_amplitude +
                          std::clamp(stride_swing, 0.0F, 1.0F) *
                              config.stride_scale;
...
auto evaluate_cycle_motion(const Dimensions &dims, const Gait &gait, float time,
  ...
  if (is_moving) {
    ...
    float const secondary =
        std::sin((sample.phase + motion.moving_secondary_phase) * 2.0F * kTwoPi);
  ...
  sample.body_sway = body_sway(is_moving, sample.phase, time,
                               sample.locomotion_intensity, gait.stride_swing,
                               sway);
