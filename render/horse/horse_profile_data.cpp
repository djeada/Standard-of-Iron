...
auto make_horse_dimensions(uint32_t seed) -> HorseDimensions {
  ...
  d.neck_length =
      ...
      k_neck_length_scale;
  d.neck_rise = rand_between(seed, k_salt_neck_rise, k_neck_rise_min, k_neck_rise_max);
  d.head_length =
  ...
  d.muzzle_length =
      rand_between(
          seed, k_salt_muzzle_length, k_muzzle_length_min, k_muzzle_length_max) *
      k_head_scale;

  d.leg_length =
      rand_between(seed, k_salt_leg_length, k_leg_length_min, k_leg_length_max);
  d.hoof_height =
      rand_between(seed, k_salt_hoof_height, k_hoof_height_min, k_hoof_height_max);

  d.tail_length =
      rand_between(seed, k_salt_tail_length, k_tail_length_min, k_tail_length_max);

  d.saddle_thickness = rand_between(
      seed, k_salt_saddle_thickness, k_saddle_thickness_min, k_saddle_thickness_max);
  d.seat_forward_offset = rand_between(seed,
                                       k_salt_seat_forward_offset,
                                       k_seat_forward_offset_min,
                                       k_seat_forward_offset_max);
  d.stirrup_out = d.body_width * rand_between(seed,
                                              k_salt_stirrup_out,
  ...
  float const leg_down_distance = d.leg_length * avg_leg_segment_ratio + d.hoof_height;
  float const shoulder_to_barrel_offset =
      d.body_height * k_shoulder_barrel_offset_scale + k_shoulder_barrel_offset_base;
...
auto make_horse_variant(uint32_t seed,
  ...
  float const leather_tone =
      rand_between(seed, k_salt_leather_tone, k_leather_tone_min, k_leather_tone_max);
  ...
  v.blanket_color =
      cloth_base *
      rand_between(seed, k_salt_blanket_tint, k_blanket_tint_min, k_blanket_tint_max);
