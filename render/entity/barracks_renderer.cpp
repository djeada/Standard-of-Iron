...
static void drawBarracks(const DrawContext& p, ISubmitter& out) {
    ...
    // Selection smoke if selected (billboarded darker particles, more and smaller)
    if (p.selected) {
        QMatrix4x4 m;
        QVector3D pos = p.model.column(3).toVector3D();
        m.translate(pos.x(), 0.0f, pos.z());
        m.scale(2.0f, 1.0f, 2.0f); // extend around the building
        // Use more, smaller, darker particles
        out.billboardSmoke(m, QVector3D(0.25f, 0.25f, 0.25f), 0.35f, 18);
    }
    // Hover smoke if hovered and not selected (subtler, fewer particles)
        ...
        out.billboardSmoke(m, QVector3D(0.25f, 0.25f, 0.25f), 0.22f, 12);
    }
}
