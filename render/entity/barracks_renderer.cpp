...
static void drawBarracks(const DrawParams& p) {
    ...
    p.renderer->queueMeshColored(p.resources->unit(), foundation, stone, p.resources->white());

    // Wooden walls (main volume)
    QMatrix4x4 walls = p.model;
    walls.scale(1.15f, 0.65f, 0.95f);
    p.renderer->queueMeshColored(p.resources->unit(), walls, wood, p.resources->white());
    ...
    p.renderer->queueMeshColored(p.resources->unit(), b, woodDark, p.resources->white());
    };
    drawBeam( 0.9f,  0.7f);
    drawBeam(-0.9f,  0.7f);
    drawBeam( 0.9f, -0.7f);
    ...
    p.renderer->queueMeshColored(p.resources->unit(), paver, path, p.resources->white());
    };
    ...
    p.renderer->queueMeshColored(p.resources->unit(), crate1, crate, p.resources->white());
    QMatrix4x4 crate2 = p.model; crate2.translate(0.58f, 0.02f, 0.45f); crate2.scale(0.14f, 0.14f, 0.14f);
    p.renderer->queueMeshColored(p.resources->unit(), crate2, crate, p.resources->white());
    ...
    p.renderer->queueMeshColored(p.resources->unit(), post, woodDark, p.resources->white());
    }
    ...
    p.renderer->queueMeshColored(p.resources->unit(), pole, woodDark, p.resources->white());

    QMatrix4x4 banner = p.model;
    banner.translate(-0.82f, 0.80f, -0.50f);
    banner.scale(0.35f, 0.22f, 0.02f);
    p.renderer->queueMeshColored(p.resources->unit(), banner, team, p.resources->white());
}
