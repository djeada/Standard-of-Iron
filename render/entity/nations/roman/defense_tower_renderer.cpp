...
auto tower_archetype() -> const RenderArchetype& {
  static const RenderArchetype k_tower = [] {
    ...
    desc.add_box(
        QVector3D(0.0F, 0.26F, 0.0F), QVector3D(1.0F, 0.02F, 1.0F), c.limestone);
    ...
    desc.add_cylinder(
        QVector3D(0.0F, 0.5F, 0.0F), QVector3D(0.0F, 2.2F, 0.0F), 0.55F, c.limestone);

    desc.add_cylinder(
        QVector3D(0.0F, 1.34F, 0.0F), QVector3D(0.0F, 1.40F, 0.0F), 0.60F, c.marble);
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      desc.add_cylinder(
          QVector3D(ox, 0.5F, oz), QVector3D(ox, 2.05F, oz), 0.09F, c.marble);
      desc.add_box(QVector3D(ox, 0.60F, oz),
                   QVector3D(0.13F, 0.09F, 0.13F),
                   c.marble,
                   BuildingStateMask::All,
                   BuildingLODMask::Full);
      desc.add_box(QVector3D(ox, 2.10F, oz),
                   QVector3D(0.14F, 0.09F, 0.14F),
                   c.marble,
                   BuildingStateMask::All,
                   BuildingLODMask::Full);
      desc.add_box(QVector3D(ox, 2.16F, oz),
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      desc.add_box(QVector3D(ox, 1.2F, oz),
                   QVector3D(0.04F, 0.22F, 0.04F),
                   c.sandstone_dark,
                   BuildingStateMask::All,
                   BuildingLODMask::Full);
    }
    ...
    desc.add_cylinder(
        QVector3D(0.0F, 2.20F, 0.0F), QVector3D(0.0F, 2.26F, 0.0F), 0.64F, c.limestone);
    desc.add_box(QVector3D(0.0F, 2.32F, 0.0F), QVector3D(0.8F, 0.05F, 0.8F), c.cedar);

    for (int i = 0; i < 8; ++i) {
      float const angle = static_cast<float>(i) * 0.785F;
      float const ox = sinf(angle) * 0.7F;
      float const oz = cosf(angle) * 0.7F;
      desc.add_box(QVector3D(ox, 2.52F, oz),
                   QVector3D(0.15F, 0.22F, 0.15F),
                   c.terracotta,
                   BuildingStateMask::All,
                   BuildingLODMask::Full);
    }
    ...
    desc.add_box(
        QVector3D(0.0F, 2.65F, 0.0F), QVector3D(0.85F, 0.04F, 0.85F), c.limestone);
    for (float x : {-0.75F, 0.75F}) {
      for (float z : {-0.75F, 0.75F}) {
        desc.add_box(QVector3D(x, 2.72F, z),
    ...
    desc.add_cylinder(QVector3D(0.0F, 2.28F, 0.0F),
                      QVector3D(0.0F, 3.20F, 0.0F),
                      0.07F,
                      c.cedar_dark);
    desc.add_palette_box(QVector3D(0.12F, 2.84F, 0.0F),
                         QVector3D(0.24F, 0.16F, 0.025F),
                         k_tower_team_slot);
    for (int i = 0; i < 2; ++i) {
      float const ring_y = 2.72F + static_cast<float>(i) * 0.32F;
      desc.add_cylinder(QVector3D(0.0F, ring_y, 0.0F),
                        QVector3D(0.0F, ring_y + 0.028F, 0.0F),
                        0.12F,
                        c.gold,
                        BuildingStateMask::All,
                        BuildingLODMask::Full);
    }
    desc.add_box(QVector3D(0.0F, 3.26F, 0.0F),
                 QVector3D(0.09F, 0.07F, 0.09F),
                 c.bronze,
                 BuildingStateMask::All,
                 BuildingLODMask::Full);
