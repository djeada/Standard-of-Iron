...
void draw_wheels(const DrawContext& p,
  ...
  QVector3D const left_front(-0.42F, wheel_radius, -0.25F);
  QVector3D const left_back(-0.42F, wheel_radius, 0.25F);

  QVector3D const right_front(0.42F, wheel_radius, -0.25F);
  QVector3D const right_back(0.42F, wheel_radius, 0.25F);
  ...
  auto draw_wheel = [&](const QVector3D& pos, float side_offset) {
    ...
    draw_cyl(out,
             p.model,
             inner - QVector3D(side_offset * 0.02F, 0, 0),
             outer + QVector3D(side_offset * 0.02F, 0, 0),
             0.04F,
             c.metal_iron,
             white);

    for (int s = 0; s < 4; ++s) {
      float const angle = s * std::numbers::pi_v<float> / 2.0F;
      ...
      QVector3D const spoke_pos =
          pos + QVector3D(side_offset * (wheel_thickness + 0.03F), spoke_y, spoke_z);
      draw_cyl(out,
               p.model,
...
void draw_throwing_arm(const DrawContext& p,
  ...
  draw_cyl(out,
           p.model,
           QVector3D(-0.25F, 0.2F, 0.0F),
           QVector3D(-0.25F, 0.65F, 0.0F),
           0.05F,
           c.wood_frame,
           white);
  draw_cyl(out,
  ...
  draw_box(out,
           ...
           white,
           arm_matrix,
           QVector3D(0.0F, -0.05F, -0.55F),
           QVector3D(0.08F, 0.06F, 0.10F),
           c.leather);
...
void draw_torsion_mechanism(const DrawContext& p,
  ...
  draw_box(out,
           ...
           white,
           p.model,
           QVector3D(-0.18F, 0.35F, 0.0F),
           QVector3D(0.04F, 0.18F, 0.15F),
           c.wood_dark);
  draw_box(out,
           unit,
           white,
  ...
  for (int i = 0; i < 3; ++i) {
    float const offset = (float(i) - 1.0F) * 0.04F;
    draw_cyl(out,
             p.model,
             QVector3D(-0.12F, 0.25F + offset, -0.08F),
             QVector3D(-0.12F, 0.45F + offset, 0.08F),
             0.025F,
             c.rope,
             white);
    draw_cyl(out,
  ...
  draw_cyl(out,
           p.model,
           QVector3D(-0.20F, 0.30F, 0.0F),
           QVector3D(-0.16F, 0.30F, 0.0F),
           0.12F,
           c.metal_iron,
           white);
  draw_cyl(out,
...
void register_catapult_renderer(EntityRendererRegistry& registry) {
  register_catapult_renderer_variant(
      registry,
      CatapultRendererConfig{.renderer_key = "troops/roman/catapult",
