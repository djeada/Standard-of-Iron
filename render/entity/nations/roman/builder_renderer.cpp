...
enum BuilderSawPaletteSlot : std::uint8_t {
  k_builder_saw_wood_slot = 0U,
  k_builder_saw_metal_slot = 1U,
  k_builder_saw_metal_dark_slot = 2U,
  k_builder_saw_leather_slot = 3U,
};

enum BuilderChiselPaletteSlot : std::uint8_t {
  k_builder_chisel_wood_slot = 0U,
...
auto builder_work_tunic_extra_role_colors(
  ...
  return base_count +
         builder_work_tunic_fill_role_colors(variant.palette, out + base_count,
...
auto roman_civilian_mantle_make_static_attachment(
    std::uint16_t chest_bone_index,
...
auto roman_civilian_mantle_extra_role_colors(
    const void *variant_void, QVector3D *out, std::uint32_t base_count,
    std::size_t max_count) -> std::uint32_t {
  if (variant_void == nullptr || max_count <= base_count) {
    return base_count;
  ...
  const auto &variant = *static_cast<const HumanoidVariant *>(variant_void);
  return base_count +
         roman_civilian_mantle_fill_role_colors(
  ...
  static const RenderArchetype archetype = [] {
    std::array<GeneratedEquipmentPrimitive, 8> const primitives{{
        generated_cylinder(QVector3D(-0.02F, -0.18F, 0.03F),
                           QVector3D(0.06F, -0.08F, 0.03F), 0.018F,
                           k_builder_saw_wood_slot),
        generated_sphere(QVector3D(0.065F, -0.07F, 0.03F), 0.024F,
                         k_builder_saw_leather_slot),
        generated_cylinder(QVector3D(0.02F, -0.06F, 0.03F),
                           QVector3D(0.03F, 0.20F, 0.03F), 0.013F,
                           k_builder_saw_metal_slot),
        generated_cylinder(QVector3D(0.03F, 0.20F, 0.03F),
                           QVector3D(-0.09F, 0.12F, 0.03F), 0.018F,
                           k_builder_saw_metal_dark_slot),
        generated_sphere(QVector3D(-0.02F, 0.16F, 0.03F), 0.016F,
                         k_builder_saw_metal_slot),
        generated_sphere(QVector3D(-0.045F, 0.145F, 0.03F), 0.014F,
                         k_builder_saw_metal_slot),
        generated_sphere(QVector3D(-0.07F, 0.13F, 0.03F), 0.012F,
                         k_builder_saw_metal_slot),
        generated_sphere(QVector3D(-0.09F, 0.115F, 0.03F), 0.010F,
                         k_builder_saw_metal_slot),
    }};
  ...
  static const RenderArchetype archetype = [] {
    std::array<GeneratedEquipmentPrimitive, 4> const primitives{{
        generated_cylinder(QVector3D(0.0F, -0.16F, 0.01F),
                           QVector3D(0.0F, 0.06F, 0.01F), 0.017F,
                           k_builder_chisel_wood_slot),
        generated_sphere(QVector3D(0.0F, -0.18F, 0.01F), 0.022F,
                         k_builder_chisel_wood_slot),
        generated_cylinder(QVector3D(0.0F, 0.06F, 0.01F),
                           QVector3D(0.0F, 0.20F, 0.01F), 0.011F,
                           k_builder_chisel_metal_slot),
        generated_sphere(QVector3D(0.0F, 0.215F, 0.01F), 0.012F,
                         k_builder_chisel_metal_slot),
    }};
    return build_generated_equipment_archetype("roman_builder_chisel",
                                               primitives);
  }();
  return archetype;
}

auto builder_tool_make_static_attachment(
    const RenderArchetype &archetype, std::uint8_t base_role_byte,
  ...
  static const auto archetype = []() {
    ...
    desc.bake_attachments[desc.bake_attachment_count++] =
        builder_work_tunic_make_static_attachment(
            static_cast<std::uint16_t>(Render::Humanoid::HumanoidBone::Chest),
            desc.role_count);
    desc.role_count = static_cast<std::uint8_t>(
        desc.role_count + k_builder_work_tunic_role_count);
...
auto roman_builder_idle_archetype() -> Render::Creature::ArchetypeId {
  static const auto archetype = []() {
    ...
    const std::array<EquipmentHandle, 4> handles{
        loadout.helmet_handle, loadout.tool_belt_handle,
        loadout.work_apron_handle, loadout.arm_guards_handle};
    ...
    if (loadout.armor_handle != k_invalid_equipment_handle) {
      register_humanoid_equipment_contribution(
          ...
          {.build_attachments = &builder_work_tunic_contribution_attachments,
           ...
           .role_count =
               static_cast<std::uint8_t>(k_builder_work_tunic_role_count)});
    }
...
auto roman_builder_tool_base_role_byte() -> std::uint8_t {
  static const auto base_role_byte = static_cast<std::uint8_t>(
      ...
      Render::GL::k_arm_guards_role_count + k_builder_work_tunic_role_count);
...
auto roman_builder_common_attachments()
  ...
  static const std::array<Render::Creature::StaticAttachmentSpec, 13>
      k_attachments{
          k_helmet_spec,         k_tool_belt_specs[0],  k_tool_belt_specs[1],
          k_tool_belt_specs[2],  k_tool_belt_specs[3],  k_tool_belt_specs[4],
          k_tool_belt_specs[5],  k_work_apron_specs[0], k_work_apron_specs[1],
          k_work_apron_specs[2], k_arm_guards_specs[0], k_arm_guards_specs[1],
          k_work_tunic_spec};
...
auto roman_builder_extra_role_colors(const void *variant_void, QVector3D *out,
  ...
  count += builder_tool_belt_fill_role_colors(v.palette, out + count,
                                              max_count - count);
  if (max_count <= count) {
    return count;
  }
  count += builder_work_apron_fill_role_colors(v.palette, out + count,
                                               max_count - count);
  if (max_count <= count) {
    return count;
  }
  count += builder_arm_guards_fill_role_colors(v.palette, out + count,
                                               max_count - count);
  if (max_count <= count) {
    return count;
  }
  count += builder_work_tunic_fill_role_colors(v.palette, out + count,
                                               max_count - count);
  return count;
}
...
auto roman_builder_hammer_unit_archetype() -> Render::Creature::ArchetypeId {
  static constexpr std::array<std::uint8_t, 3> k_slots{
      k_builder_hammer_wood_slot, k_builder_hammer_metal_slot,
      k_builder_hammer_metal_dark_slot};
  ...
  static const auto k_archetype = register_builder_tool_variant_archetype(
      ...
      roman_builder_idle_archetype(), k_tool_spec,
      +[](const void *variant_void, QVector3D *out, std::uint32_t base_count,
          std::size_t max_count) -> std::uint32_t {
        if (variant_void == nullptr || max_count <= base_count) {
          return base_count;
        ...
        const auto &v = *static_cast<const HumanoidVariant *>(variant_void);
        return base_count +
               builder_hammer_fill_role_colors(v.palette, out + base_count,
                                               max_count - base_count);
      },
...
auto roman_builder_saw_unit_archetype() -> Render::Creature::ArchetypeId {
  static constexpr std::array<std::uint8_t, 4> k_slots{
      k_builder_saw_wood_slot, k_builder_saw_metal_slot,
      k_builder_saw_metal_dark_slot, k_builder_saw_leather_slot};
  static const auto k_tool_spec = builder_tool_make_static_attachment(
      ...
      k_slots);
  static const auto k_archetype = register_builder_tool_variant_archetype(
      "troops/roman/builder/construction_saw", roman_builder_idle_archetype(),
      k_tool_spec,
      +[](const void *variant_void, QVector3D *out, std::uint32_t base_count,
          std::size_t max_count) -> std::uint32_t {
        if (variant_void == nullptr || max_count <= base_count) {
        ...
        const auto &v = *static_cast<const HumanoidVariant *>(variant_void);
        return base_count +
               builder_saw_fill_role_colors(v.palette, out + base_count,
                                            max_count - base_count);
      },
...
auto roman_builder_chisel_unit_archetype() -> Render::Creature::ArchetypeId {
  static constexpr std::array<std::uint8_t, 2> k_slots{
      k_builder_chisel_wood_slot, k_builder_chisel_metal_slot};
  static const auto k_tool_spec = builder_tool_make_static_attachment(
  ...
  static const auto k_archetype = register_builder_tool_variant_archetype(
      ...
      +[](const void *variant_void, QVector3D *out, std::uint32_t base_count,
        ...
        const auto &v = *static_cast<const HumanoidVariant *>(variant_void);
        return base_count +
               builder_chisel_fill_role_colors(v.palette, out + base_count,
                                               max_count - base_count);
      },
...
void apply_roman_civilian_palette(const QVector3D &team_tint,
  ...
  variant.palette.cloth = mix_palette_color(variant.palette.cloth, tunic_color,
                                            team_tint, 0.10F, 0.90F);
  variant.palette.leather = mix_palette_color(
      variant.palette.leather, sash_color, team_tint, 0.08F, 0.92F);
