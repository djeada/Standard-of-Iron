...
public:
  ...
  void customize_pose(const DrawContext &ctx,
    ...
    float const forward_lean = (dims.seatForwardOffset * 0.08F + speed_lean) / 0.15F; // Normalize to [0,1] range
    mounted_controller.ridingLeaning(mount, forward_lean, 0.0F);

    if (anim.is_attacking && anim.isMelee) {
      // Use MountedPoseController for melee combat
      float const attack_phase =
          std::fmod(anim.time * MOUNTED_KNIGHT_INV_ATTACK_CYCLE_TIME, 1.0F);
      mounted_controller.ridingMeleeStrike(mount, attack_phase);
    } else {
      // Use riding idle with rein slack based on speed
      float const rein_slack = std::clamp(speed_norm * 0.5F, 0.0F, 0.8F);
      mounted_controller.holdReins(mount, rein_slack, rein_slack);
    }
  }
