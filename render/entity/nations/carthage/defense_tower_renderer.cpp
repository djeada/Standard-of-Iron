...
auto tower_archetype() -> const RenderArchetype& {
  static const RenderArchetype k_tower = [] {
    ...
    desc.add_box(
        QVector3D(0.0F, 0.5F, 0.0F), QVector3D(0.9F, 0.1F, 0.9F), c.stone_light);

    desc.add_box(
        QVector3D(0.0F, 1.2F, 0.0F), QVector3D(0.75F, 0.7F, 0.75F), c.stone_light);

    desc.add_box(QVector3D(0.0F, 1.18F, 0.0F),
                 QVector3D(0.78F, 0.04F, 0.78F),
                 c.stone_dark,
                 BuildingStateMask::All,
                 BuildingLODMask::Full);
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      desc.add_cylinder(
          QVector3D(ox, 0.4F, oz), QVector3D(ox, 2.05F, oz), 0.15F, c.stone_dark);
      desc.add_box(QVector3D(ox, 2.12F, oz),
                   QVector3D(0.20F, 0.07F, 0.20F),
                   c.stone_light,
                   BuildingStateMask::All,
                   BuildingLODMask::Full);
    }
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      desc.add_box(QVector3D(ox, 0.9F, oz), QVector3D(0.12F, 0.4F, 0.12F), c.brick);
    }
    desc.add_box(
        QVector3D(0.0F, 1.65F, 0.0F), QVector3D(0.82F, 0.08F, 0.82F), c.brick_dark);

    desc.add_box(
        QVector3D(0.0F, 1.94F, 0.0F), QVector3D(1.00F, 0.04F, 1.00F), c.stone_dark);
    desc.add_box(QVector3D(0.0F, 2.00F, 0.0F), QVector3D(0.95F, 0.05F, 0.95F), c.wood);

    for (int i = 0; i < 8; ++i) {
      float const angle = static_cast<float>(i) * 0.785F;
      float const ox = sinf(angle) * 0.82F;
      float const oz = cosf(angle) * 0.82F;
      desc.add_box(QVector3D(ox, 2.19F, oz),
                   QVector3D(0.13F, 0.22F, 0.13F),
                   c.brick,
                   BuildingStateMask::All,
                   BuildingLODMask::Full);
    }
    desc.add_box(
        QVector3D(0.0F, 2.40F, 0.0F), QVector3D(1.0F, 0.03F, 1.0F), c.tile_red);

    desc.add_cylinder(
        QVector3D(0.0F, 2.10F, 0.0F), QVector3D(0.0F, 3.00F, 0.0F), 0.08F, c.wood_dark);
    desc.add_palette_box(QVector3D(0.16F, 2.70F, 0.0F),
                         QVector3D(0.26F, 0.19F, 0.025F),
                         k_tower_team_slot);
    for (int i = 0; i < 3; ++i) {
      float const ring_y = 2.38F + static_cast<float>(i) * 0.26F;
      desc.add_cylinder(QVector3D(0.0F, ring_y, 0.0F),
                        QVector3D(0.0F, ring_y + 0.032F, 0.0F),
                        0.13F,
                        c.iron,
                        BuildingStateMask::All,
                        BuildingLODMask::Full);
    }
    desc.add_box(QVector3D(0.0F, 3.07F, 0.0F),
                 QVector3D(0.11F, 0.09F, 0.11F),
                 c.iron,
                 BuildingStateMask::All,
                 BuildingLODMask::Full);
