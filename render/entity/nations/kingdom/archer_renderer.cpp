...
public:
  ...
  void addAttachments(const DrawContext &ctx, const HumanoidVariant &v,
    ...
    if (bow) {
      ...
      bow_config.bow_top_y = HP::SHOULDER_Y + 0.65F;   // Taller than other bows
      bow_config.bow_bot_y = HP::WAIST_Y - 0.35F;      // Extends lower
      bow_config.bow_x = 0.0F;
      bow_config.bow_depth = 0.20F;          // Less curve than composite
      bow_config.bow_curve_factor = 0.8F;    // Straighter longbow style
      bow_config.bow_height_scale = 1.15F;   // Taller bow
