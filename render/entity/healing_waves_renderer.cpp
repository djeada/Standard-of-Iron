    ...
    for (int wave_idx = 0; wave_idx < k_num_waves; ++wave_idx) {
      ...
      for (int ribbon = 0; ribbon < k_wave_ribbons; ++ribbon) {
        ...
        for (int seg = 0; seg < k_segments_per_wave; ++seg) {
          ...
          float seg_angle =
              ribbon_angle_offset + seg_dist * k_spiral_twist_rate + spiral_phase;
          float next_seg_angle =
              ribbon_angle_offset + next_seg_dist * k_spiral_twist_rate + spiral_phase;
          ...
          float seg_intensity =
              wave_intensity * (1.0F - std::abs(seg_t - 0.5F) * k_edge_fade_factor);
          seg_intensity = std::max(0.0F, seg_intensity);

          if (seg_intensity > 0.01F) {
            renderer->healing_beam(seg_pos,
                                   next_seg_pos,
                                   color,
                                   1.0F,
                                   0.04F,
