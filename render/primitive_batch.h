...
public:
  ...
  void
  add_sphere(const QMatrix4x4& transform, const QVector3D& color, float alpha = 1.0F);
  void
  add_cylinder(const QMatrix4x4& transform, const QVector3D& color, float alpha = 1.0F);
  void
  add_cone(const QMatrix4x4& transform, const QVector3D& color, float alpha = 1.0F);

  [[nodiscard]] auto sphere_count() const -> std::size_t { return m_spheres.size(); }
  ...
  [[nodiscard]] auto sphere_data() const -> const std::vector<PrimitiveInstanceGpu>& {
    return m_spheres;
  }
  [[nodiscard]] auto cylinder_data() const -> const std::vector<PrimitiveInstanceGpu>& {
    return m_cylinders;
  }
  [[nodiscard]] auto cone_data() const -> const std::vector<PrimitiveInstanceGpu>& {
    return m_cones;
  }
