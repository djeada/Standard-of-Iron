...
inline auto single_cylinder_archetype(
    float radius,
    int material_id,
    std::string_view debug_prefix = "segment_cylinder") -> const RenderArchetype& {
  ...
  cache.push_back({std::string(debug_prefix),
                   radius_key,
                   material_id,
                   build_generated_equipment_archetype(
                       std::string(debug_prefix) + "_" + std::to_string(radius_key) +
