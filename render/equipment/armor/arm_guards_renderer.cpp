...
auto arm_guard_transform(const QMatrix4x4& parent,
                         const QVector3D& elbow,
                         const QVector3D& wrist) -> QMatrix4x4 {
  QVector3D const arm_dir = (wrist - elbow).normalized();
  QVector3D side_axis = QVector3D::crossProduct(QVector3D(0.0F, 0.0F, 1.0F), arm_dir);
...
auto arm_guards_archetype(const ArmGuardsConfig& config,
  ...
  RenderArchetypeBuilder builder{"arm_guards_" + std::to_string(arm_length_key) + "_" +
  ...
  cache.push_back({arm_length_key,
                   guard_length_key,
                   config.include_straps,
...
void ArmGuardsRenderer::render(const DrawContext& ctx,
...
void ArmGuardsRenderer::render_arm_guard(const ArmGuardsConfig& config,
  ...
  append_equipment_archetype(batch,
                             arm_guards_archetype(config, arm_length),
