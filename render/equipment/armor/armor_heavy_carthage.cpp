...
auto armor_heavy_carthage_archetype(const std::array<QMatrix4x4, 3>& torsos)
  ...
  builder.add_palette_mesh(
      torso_mesh, torsos[1], k_carthage_heavy_bronze_slot, nullptr, 1.0F, 1);
  builder.add_palette_mesh(
...
void ArmorHeavyCarthageRenderer::submit(const ArmorHeavyCarthageConfig&,
  ...
  float const torso_depth = (torso.depth > 0.0F) ? torso.depth : torso.radius * 0.75F;
  auto depth_scale_for = [&](float base) {
    float const ratio = torso_depth / std::max(0.001F, torso_r);
    return std::max(0.08F, base * ratio);
  };
  float const waist_r = waist.radius > 0.0F ? waist.radius : torso.radius * 0.90F;
  ...
  auto build_torso = [&](const QVector3D& a,
                         ...
                         float scale_x,
                         float base_z) {
    QMatrix4x4 m = oriented_cylinder(torso_local.point(a),
                                     torso_local.point(b),
  ...
  std::array<QVector3D, 3> const palette_slots{
      chainmail_color, bronze_color, bronze_core};
