...
auto tool_belt_ring_archetype() -> const RenderArchetype& {
  static const RenderArchetype archetype = [] {
    ...
    for (int i = 0; i < segs; ++i) {
      ...
      primitives[i] =
          generated_cylinder(QVector3D(std::sin(a1), k_belt_y_norm, std::cos(a1)),
                             QVector3D(std::sin(a2), k_belt_y_norm, std::cos(a2)),
                             k_belt_thickness_norm,
                             k_leather_slot);
    }
...
auto tool_belt_buckle_archetype() -> const RenderArchetype& {
  static const RenderArchetype archetype = [] {
    std::array<GeneratedEquipmentPrimitive, 2> const primitives{{
        generated_sphere(
            QVector3D(-0.18F, k_belt_y_norm, 0.92F), k_small_sphere_norm, k_metal_slot),
...
auto tool_belt_hammer_archetype() -> const RenderArchetype& {
  static const RenderArchetype archetype = [] {
    std::array<GeneratedEquipmentPrimitive, 5> const primitives{{
        generated_sphere(
            QVector3D(-0.891F, -0.179F, 0.454F), 0.050F, k_leather_dark_slot),
        generated_sphere(
            QVector3D(-0.891F, -0.357F, 0.454F), 0.039F, k_leather_dark_slot),
        generated_sphere(
            QVector3D(-0.891F, -0.536F, 0.454F), 0.029F, k_leather_dark_slot),
        generated_cylinder(QVector3D(-0.891F, -0.286F, 0.454F),
                           QVector3D(-0.891F, -0.714F, 0.454F),
                           0.029F,
                           k_wood_slot),
        generated_cylinder(QVector3D(-0.980F, -0.232F, 0.454F),
                           QVector3D(-0.802F, -0.232F, 0.454F),
                           0.043F,
                           k_metal_dark_slot),
    }};
...
auto tool_belt_chisel_archetype() -> const RenderArchetype& {
  static const RenderArchetype archetype = [] {
    std::array<GeneratedEquipmentPrimitive, 3> const primitives{{
        generated_sphere(
            QVector3D(0.809F, -0.143F, 0.588F), 0.064F, k_leather_dark_slot),
        generated_cylinder(QVector3D(0.809F, -0.214F, 0.588F),
                           QVector3D(0.809F, 0.143F, 0.588F),
                           0.021F,
                           k_metal_dark_slot),
        generated_sphere(QVector3D(0.809F, 0.143F, 0.588F), 0.029F, k_metal_slot),
    }};
  ...
  static const RenderArchetype archetype = [] {
    std::array<GeneratedEquipmentPrimitive, 7> const primitives{{
        generated_cylinder(QVector3D(0.534F, -0.161F, -0.802F),
                           QVector3D(0.695F, -0.286F, -0.910F),
                           0.029F,
                           k_wood_slot),
        generated_cylinder(QVector3D(0.704F, -0.286F, -0.919F),
                           QVector3D(0.945F, -0.455F, -1.035F),
                           0.024F,
                           k_metal_dark_slot),
        generated_sphere(
            QVector3D(0.625F, -0.232F, -0.865F), 0.037F, k_leather_dark_slot),
        generated_sphere(QVector3D(0.792F, -0.357F, -0.965F), 0.018F, k_metal_slot),
        generated_sphere(QVector3D(0.847F, -0.393F, -0.991F), 0.018F, k_metal_slot),
        generated_sphere(QVector3D(0.901F, -0.429F, -1.017F), 0.018F, k_metal_slot),
        generated_sphere(QVector3D(0.956F, -0.464F, -1.043F), 0.018F, k_metal_slot),
    }};
  ...
  static const RenderArchetype archetype = [] {
    ...
    for (int side = 0; side < 2; ++side) {
      ...
      QVector3D const pouch_pos{
          0.95F * std::sin(pouch_angle), -0.214F, 0.85F * std::cos(pouch_angle)};
      for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 3; ++j) {
          ...
          primitives[index++] =
              generated_sphere(pouch_pos + QVector3D(x_off * side_sign, -y_off, 0.0F),
                               radius,
                               k_leather_dark_slot);
        }
      }
...
auto tool_belt_palette(const ToolBeltConfig& config) -> std::array<QVector3D, 5> {
  return {config.leather_color,
          config.leather_color * 0.90F,
          config.metal_color,
          config.metal_color * 0.92F,
          config.wood_color};
}
...
void ToolBeltRenderer::render(const DrawContext& ctx,
...
void ToolBeltRenderer::submit(const ToolBeltConfig& config,
  ...
  append_humanoid_attachment_archetype_scaled(batch,
                                              ctx,
                                              waist,
                                              tool_belt_ring_archetype(),
                                              QVector3D(waist_r, waist.radius, waist_d),
                                              palette);
  append_humanoid_attachment_archetype(
      batch, ctx, waist, tool_belt_buckle_archetype(), palette);

  if (config.include_hammer) {
    append_humanoid_attachment_archetype(
        batch, ctx, waist, tool_belt_hammer_archetype(), palette);
  }
  if (config.include_chisel) {
    append_humanoid_attachment_archetype(
        batch, ctx, waist, tool_belt_chisel_archetype(), palette);
  }
  if (config.include_saw) {
    append_humanoid_attachment_archetype(
        batch, ctx, waist, tool_belt_saw_archetype(), palette);
  }
...
auto tool_belt_make_static_attachments(std::uint16_t waist_socket_bone_index,
  ...
  auto make_spec = [&](const RenderArchetype& arch, const QMatrix4x4& bind_mat) {
