...
auto roman_shield_local_pose() -> QMatrix4x4 {
  QVector3D const shield_center_local{-k_shield_width * 0.40F, 0.06F, 0.05F};
  QMatrix4x4 pose;
  pose.translate(shield_center_local);
...
void RomanShieldRenderer::submit(const RomanShieldConfig &,
  ...
  append_equipment_archetype(batch, roman_shield_archetype(),
                             hand_basis_transform(ctx.model, grip) *
                                 roman_shield_local_pose(),
                             palette_slots);
}
