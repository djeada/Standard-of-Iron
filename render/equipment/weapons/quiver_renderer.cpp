...
auto quiver_body_archetype(const QuiverRenderConfig& config) -> const RenderArchetype& {
  ...
  std::array<GeneratedEquipmentPrimitive, 1> const primitives{{
      generated_cylinder(QVector3D(0.0F, -0.25F, 0.05F),
                         QVector3D(0.0F, 0.15F, -0.10F),
                         config.quiver_radius,
                         k_leather_slot,
                         1.0F,
                         config.material_id),
  }};
...
auto quiver_palette(const QuiverRenderConfig& config,
                    const HumanoidPalette& palette) -> std::array<QVector3D, 3> {
  return {palette.leather, palette.wood, config.fletching_color};
}

void append_quiver_arrow(EquipmentBatch& batch,
  ...
  QVector3D const shaft_tip = waist_basis_point(waist, q_top_local + shaft_local_end);
  ...
  append_equipment_archetype(
      ...
      oriented_segment_transform(ctx.model, arrow_origin, shaft_axis),
      palette);
  append_equipment_archetype(
      batch,
...
void QuiverRenderer::submit(const QuiverRenderConfig& config,
  ...
  QVector3D const q_top_local = quiver_pos_local + QVector3D(0.0F, 0.15F, -0.10F);
  auto const palette_slots = quiver_palette(config, palette);

  append_equipment_archetype(batch,
                             quiver_body_archetype(config),
                             waist_basis_transform(ctx.model, waist, quiver_pos_local),
                             palette_slots);
  ...
  append_quiver_arrow(
      batch, ctx, waist, config, palette_slots, q_top_local, QVector3D(j, 0.08F, k));

  if (config.num_arrows >= 2) {
    append_quiver_arrow(batch,
                        ctx,
                        waist,
                        config,
                        palette_slots,
...
auto quiver_make_static_attachments(const QuiverRenderConfig& config,
  ...
  const AttachmentFrame& bw = Render::Humanoid::humanoid_bind_body_frames().waist;
  QVector3D const quiver_pos_local(bw.radius * 0.9F + 0.15F, -0.10F, 0.0F);
  QVector3D const q_top_local = quiver_pos_local + QVector3D(0.0F, 0.15F, -0.10F);

  auto make_spec =
      [&](const RenderArchetype& arch,
          const QMatrix4x4& unit_local_transform,
  ...
  QVector3D const arrow1_tip_UL = waist_basis_point(bw, q_top_local + shaft1_end_local);
  ...
  QMatrix4x4 const fletching1_transform =
      waist_basis_transform(QMatrix4x4{}, bw, q_top_local + shaft1_end_local) * scale1;

  QVector3D const shaft2_end_local(0.02F, 0.07F, 0.02F);
  QVector3D const arrow2_origin_UL = waist_basis_point(bw, q_top_local);
  QVector3D const arrow2_tip_UL = waist_basis_point(bw, q_top_local + shaft2_end_local);
  ...
  QMatrix4x4 const fletching2_transform =
      waist_basis_transform(QMatrix4x4{}, bw, q_top_local + shaft2_end_local) * scale2;

  return {
      make_spec(
          quiver_body_archetype(config), body_transform, leather_role, leather_role),
      make_spec(arrow_shaft_archetype(0.010F, config.material_id, "quiver_shaft"),
                shaft1_transform,
                leather_role,
                fletching_role),
      make_spec(
          arrow_fletching_archetype(0.025F, config.material_id, "quiver_fletching"),
          fletching1_transform,
          leather_role,
          fletching_role),
      make_spec(arrow_shaft_archetype(0.010F, config.material_id, "quiver_shaft"),
                shaft2_transform,
