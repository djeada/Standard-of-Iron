...
auto carthage_shield_archetype(float scale_multiplier) -> const RenderArchetype& {
  ...
  float const shield_radius = (k_base_shield_diameter * 0.5F) * scale_multiplier;
  float const dome_depth = shield_radius * 0.48F;
  QVector3D const shield_center = shield_center_local(shield_radius);

  RenderArchetypeBuilder builder{"carthage_shield_" + std::to_string(scale_key)};

  QMatrix4x4 dome;
  dome.translate(shield_center);
  dome.scale(shield_radius, shield_radius, dome_depth);
  builder.add_palette_mesh(
      get_unit_hemisphere_mesh(), dome, k_shield_slot, nullptr, 1.0F, 4);

  constexpr int rim_segments = 24;
  for (int i = 0; i < rim_segments; ++i) {
    float const a0 = (static_cast<float>(i) / static_cast<float>(rim_segments)) * 2.0F *
                     std::numbers::pi_v<float>;
    float const a1 = (static_cast<float>(i + 1) / static_cast<float>(rim_segments)) *
                     2.0F * std::numbers::pi_v<float>;

    QVector3D const p0 =
        shield_center +
        QVector3D(shield_radius * std::cos(a0), shield_radius * std::sin(a0), 0.0F);
    QVector3D const p1 =
        shield_center +
        QVector3D(shield_radius * std::cos(a1), shield_radius * std::sin(a1), 0.0F);
    builder.add_palette_mesh(get_unit_cylinder(),
                             cylinder_between(p0, p1, 0.012F),
                             k_trim_slot,
                             nullptr,
                             1.0F,
                             4);
  ...
  builder.add_palette_mesh(
      get_unit_cylinder(),
      cylinder_between(
          emblem_plane + QVector3D(-shield_radius * 0.22F, shield_radius * 0.02F, 0.0F),
          emblem_plane + QVector3D(shield_radius * 0.22F, shield_radius * 0.02F, 0.0F),
          emblem_radius * 0.75F),
      k_metal_slot,
      nullptr,
  ...
  builder.add_palette_mesh(
      get_unit_cone(),
      cone_from_to(emblem_body_bot - QVector3D(0.0F, shield_radius * 0.04F, 0.0F),
...
auto carthage_shield_make_static_attachment(const CarthageShieldConfig& config,
  ...
  spec.palette_role_remap[k_grip_slot] = static_cast<std::uint8_t>(base_role_byte + 3U);
  return spec;
}
