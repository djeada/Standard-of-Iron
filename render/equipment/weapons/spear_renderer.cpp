...
auto Render::GL::spear_make_static_attachments(
    const SpearRenderConfig &config, std::uint16_t socket_bone_index,
    std::uint8_t base_role_byte, const QMatrix4x4 &bind_hand_r_matrix,
    const QVector3D &bind_hand_r_right)
    -> std::array<Render::Creature::StaticAttachmentSpec, 4> {
  QVector3D const grip_pos = bind_hand_r_matrix.column(3).toVector3D();
  ...
  QVector3D right_hint = bind_hand_r_right;
  if (right_hint.lengthSquared() < 1e-6F) {
    right_hint = QVector3D(0.0F, 0.0F, 1.0F);
  }
  ...
  auto make_spec = [&](const RenderArchetype &arch, const QVector3D &from,
    ...
    return Render::Equipment::build_static_attachment({
        ...
        .unit_local_pose_at_bind = oriented_segment_transform(
            QMatrix4x4{}, from, to - from, right_hint),
        .palette_role_remap =
