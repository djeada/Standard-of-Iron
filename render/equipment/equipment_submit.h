...
struct EquipmentBatch {
  ...
  void reserve(std::size_t mesh_count,
               std::size_t cyl_count = 0,
...
void submit_equipment_batch(const EquipmentBatch& batch, ISubmitter& out) noexcept;

class BatchSubmitterAdapter : public ISubmitter {
public:
  explicit BatchSubmitterAdapter(EquipmentBatch& batch)
      : m_batch(batch) {}
  ...
  void mesh(Mesh* mesh,
    ...
    m_batch.meshes.push_back({mesh, nullptr, model, color, tex, alpha, material_id});
  }
  ...
  void part(Mesh* mesh,
            ...
            float alpha = 1.0F,
            int material_id = 0) override {
    m_batch.meshes.push_back({mesh, material, model, color, tex, alpha, material_id});
  }
