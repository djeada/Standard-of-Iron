...
inline auto
make_humanoid_attachment_transform_scaled(const QMatrix4x4& parent,
                                          const AttachmentFrame& frame,
                                          const QVector3D& local_offset,
                                          const QVector3D& axis_scale) -> QMatrix4x4 {
  return make_basis_attachment_transform_scaled(parent,
                                                frame.origin,
                                                frame.right,
                                                frame.up,
                                                frame.forward,
...
inline auto make_humanoid_attachment_transform(const QMatrix4x4& parent,
                                               const AttachmentFrame& frame,
                                               const QVector3D& local_offset,
  ...
  return make_humanoid_attachment_transform_scaled(
      ...
      frame,
      local_offset,
      QVector3D(frame.radius * uniform_scale,
...
inline void append_humanoid_attachment_archetype(
  ...
  append_equipment_archetype(
      batch,
      archetype,
      make_humanoid_attachment_transform(ctx.model, frame, local_offset, uniform_scale),
      palette,
      default_texture,
      alpha_multiplier,
      lod);
