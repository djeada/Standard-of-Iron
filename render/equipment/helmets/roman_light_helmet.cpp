...
auto roman_light_helmet_archetype() -> const RenderArchetype& {
  static const RenderArchetype archetype = [] {
    ...
    constexpr int k_total = 16 + k_crest_fiber_count;
    std::array<GeneratedEquipmentPrimitive, k_total> primitives{};

    primitives[0] =
        generated_sphere(QVector3D(0.0F, 0.62F, -0.04F), 1.46F, k_metal_slot, 1.0F, 2);
    ...
    for (int i = 0; i < k_crest_fiber_count; ++i) {
      ...
      primitives[16 + i] = generated_cylinder(base, tip, 0.05F, k_crest_slot, 0.92F, 0);
    }
