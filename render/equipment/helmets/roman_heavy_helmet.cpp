...
void RomanHeavyHelmetRenderer::render(const DrawContext &ctx,
  ...
  submitter.mesh(getUnitCylinder(),
                 ...
                 steel_color * 1.06F, nullptr, 1.0F);
  ...
  submitter.mesh(getUnitCylinder(),
                 ...
                 brass_accent * 0.92F, nullptr, 1.0F);

  // Simplified cheek guards - shader adds strap holes and embossing
  float const cheek_w = head_r * 0.56F;
  QVector3D const cheek_top = headPoint(QVector3D(0.0F, 0.15F, 0.0F));
  QVector3D const cheek_bot = headPoint(QVector3D(0.0F, -0.48F, 0.0F));

  QVector3D const cheek_ltop =
      cheek_top + head.right * (-cheek_w / head_r) + head.forward * 0.45F;
  QVector3D const cheek_lbot = cheek_bot +
                               head.right * (-cheek_w * 0.88F / head_r) +
                               head.forward * 0.35F;
  submitter.mesh(getUnitCylinder(),
                 cylinderBetween(ctx.model, cheek_lbot, cheek_ltop, 0.036F),
  ...
  QVector3D const cheek_rbot = cheek_bot +
                               ...
                               head.forward * 0.35F;
  submitter.mesh(getUnitCylinder(),
                 cylinderBetween(ctx.model, cheek_rbot, cheek_rtop, 0.036F),
                 steel_color * 0.92F, nullptr, 1.0F);
  ...
  submitter.mesh(getUnitCylinder(),
                 ...
                 steel_color * 0.88F, nullptr, 1.0F);
