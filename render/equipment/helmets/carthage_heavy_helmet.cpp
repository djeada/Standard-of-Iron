  ...
  static const RenderArchetype archetype = [] {
    std::array<GeneratedEquipmentPrimitive, 1> const primitives{{
        generated_cylinder(QVector3D(0.0F, -0.36F, -1.00F),
                           QVector3D(0.0F, -0.14F, -1.06F),
                           1.42F,
                           k_metal_dark_slot,
                           1.0F,
                           2),
    }};
    return build_generated_equipment_archetype("carthage_heavy_neck_guard", primitives);
  }();
  return archetype;
}
  ...
  static const RenderArchetype archetype = [] {
    std::array<GeneratedEquipmentPrimitive, 1> const primitives{{
        generated_cylinder(QVector3D(0.0F, -0.06F, 0.90F),
                           QVector3D(0.0F, 0.58F, 0.70F),
                           0.10F,
                           k_metal_slot,
                           1.0F,
                           2),
    }};
    return build_generated_equipment_archetype("carthage_heavy_face_plate", primitives);
  }();
  return archetype;
}
...
auto carthage_heavy_helmet_crest_archetype() -> const RenderArchetype& {
  static const RenderArchetype archetype = [] {
    ...
    for (int i = 0; i < 12; ++i) {
      ...
      primitives[1 + i] = generated_cylinder(base, tip, 0.052F, k_crest_slot, 0.92F, 0);
    }

    return build_generated_equipment_archetype("carthage_heavy_crest", primitives);
  }();
  return archetype;
}
...
auto carthage_heavy_helmet_rivets_archetype() -> const RenderArchetype& {
  static const RenderArchetype archetype = [] {
    std::array<GeneratedEquipmentPrimitive, 5> const primitives{{
        generated_sphere(
            QVector3D(-0.52F, 0.28F, 0.64F), 0.05F, k_metal_light_slot, 1.0F, 2),
        generated_sphere(
            QVector3D(-0.26F, 0.28F, 0.64F), 0.05F, k_metal_light_slot, 1.0F, 2),
        generated_sphere(
            QVector3D(0.0F, 0.28F, 0.64F), 0.05F, k_metal_light_slot, 1.0F, 2),
        generated_sphere(
            QVector3D(0.26F, 0.28F, 0.64F), 0.05F, k_metal_light_slot, 1.0F, 2),
        generated_sphere(
            QVector3D(0.52F, 0.28F, 0.64F), 0.05F, k_metal_light_slot, 1.0F, 2),
    }};
    return build_generated_equipment_archetype("carthage_heavy_rivets", primitives);
  }();
  return archetype;
}
...
auto carthage_heavy_helmet_fill_role_colors(const HumanoidPalette& palette,
  ...
  if (max < k_carthage_heavy_helmet_role_count) {
    return 0;
  }
  auto const colors = carthage_heavy_palette(palette, CarthageHeavyHelmetConfig{});
  out[0] = colors[0];
  out[1] = colors[1];
  out[2] = colors[2];
  out[3] = colors[3];
  return k_carthage_heavy_helmet_role_count;
}
...
void CarthageHeavyHelmetRenderer::submit(const CarthageHeavyHelmetConfig& config,
