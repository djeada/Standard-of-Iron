...
auto headwrap_fill_role_colors(const HumanoidPalette& palette,
                               QVector3D* out,
...
auto headwrap_make_static_attachment(std::uint16_t socket_bone_index,
  ...
  spec.palette_role_remap[k_knot_slot] = static_cast<std::uint8_t>(base_role_byte + 1U);
  spec.palette_role_remap[k_tail_slot] = static_cast<std::uint8_t>(base_role_byte + 2U);
  return spec;
}

void HeadwrapRenderer::render(const DrawContext& ctx,
...
void HeadwrapRenderer::submit(const HeadwrapConfig&,
