...
void add_rein_segments(RenderArchetypeBuilder& builder,
  ...
  builder.add_palette_cylinder(
      bits[0], bits[1], k_rein_radius, k_rein_slot, nullptr, 1.0F, 4);
}
...
auto reins_fill_role_colors(const HorseVariant& variant,
                            QVector3D* out,
...
auto reins_make_static_attachment(std::uint16_t socket_bone_index,
  ...
  auto spec = make_static_attachment_spec_from_horse_renderer(
      reins_archetype(), socket_bone_index, bind_pose_frame, bind_palette_socket_bone);
