...
auto blanket_fill_role_colors(const HorseVariant& variant,
                              QVector3D* out,
...
auto blanket_make_static_attachment(std::uint16_t socket_bone_index,
  ...
  auto spec = make_static_attachment_spec_from_horse_renderer(blanket_archetype(),
                                                              socket_bone_index,
                                                              bind_pose_frame,
...
void BlanketRenderer::submit(const DrawContext& ctx,
  ...
  append_horse_attachment_archetype(
      batch, ctx, frames.back_center, blanket_archetype(), palette);
}
