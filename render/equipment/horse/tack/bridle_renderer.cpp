...
void BridleRenderer::submit(const DrawContext &ctx,
  ...
  batch.cylinders.push_back(
      {headband_start, headband_end, 0.012F, variant.tack_color, 1.0F});
  ...
  QVector3D const noseband_end =
      muzzle.origin - muzzle.right * 0.20F + muzzle.forward * 0.10F;
  batch.cylinders.push_back(
      {noseband_start, noseband_end, 0.010F, variant.tack_color, 1.0F});
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    batch.cylinders.push_back(
        {cheek_top, cheek_bottom, 0.011F, variant.tack_color, 1.0F});
  }

  QVector3D const throatlatch_start = head.origin - head.up * 0.10F;
  QVector3D const throatlatch_end = head.origin - head.up * 0.25F;
  batch.cylinders.push_back(
      {throatlatch_start, throatlatch_end, 0.010F, variant.tack_color, 1.0F});
}
