...
auto bridle_archetype() -> const RenderArchetype& {
  static const RenderArchetype archetype = [] {
    ...
    QVector3D const headband_start = head.origin + head.right * 0.22F + head.up * 0.15F;
    QVector3D const headband_end = head.origin - head.right * 0.22F + head.up * 0.15F;
    add_bridle_segment(builder, head, headband_start, headband_end, k_headband_radius);

    QVector3D const noseband_start =
        muzzle.origin + muzzle.right * 0.20F + muzzle.forward * 0.10F;
    QVector3D const noseband_end =
        muzzle.origin - muzzle.right * 0.20F + muzzle.forward * 0.10F;
    add_bridle_segment(builder, head, noseband_start, noseband_end, k_noseband_radius);
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      add_bridle_segment(builder, head, cheek_top, cheek_bottom, k_cheek_radius);
    }

    QVector3D const throatlatch_start = head.origin - head.up * 0.10F;
    QVector3D const throatlatch_end = head.origin - head.up * 0.25F;
    add_bridle_segment(
        builder, head, throatlatch_start, throatlatch_end, k_throatlatch_radius);
    return std::move(builder).build();
  }();
  return archetype;
}

auto bridle_fill_role_colors(const HorseVariant& variant,
                             QVector3D* out,
...
auto bridle_make_static_attachment(std::uint16_t socket_bone_index,
  ...
  auto spec = make_static_attachment_spec_from_horse_renderer(
      bridle_archetype(), socket_bone_index, bind_pose_frame, bind_palette_socket_bone);
...
void BridleRenderer::submit(const DrawContext& ctx,
  ...
  append_horse_attachment_archetype(
      batch, ctx, frames.head, bridle_archetype(), palette);
}
