...
auto saddle_bag_fill_role_colors(const HorseVariant& variant,
                                 QVector3D* out,
...
auto saddle_bag_make_static_attachment(std::uint16_t socket_bone_index,
                                       std::uint8_t base_role_byte,
                                       const HorseAttachmentFrame& bind_pose_frame,
                                       const QMatrix4x4& bind_palette_socket_bone)
    -> Render::Creature::StaticAttachmentSpec {
  auto spec = make_static_attachment_spec_from_horse_renderer(saddle_bag_archetype(),
                                                              socket_bone_index,
                                                              bind_pose_frame,
...
void SaddleBagRenderer::submit(const DrawContext& ctx,
  ...
  append_horse_attachment_archetype(
      batch, ctx, frames.back_center, saddle_bag_archetype(), palette);
}
