...
void ChampionRenderer::render(const DrawContext &ctx,
  ...
  batch.meshes.push_back({get_unit_sphere(), nullptr, plate_center, armor_color, nullptr, 1.0F, 1});
  ...
  batch.meshes.push_back({get_unit_sphere(), nullptr, upper_plate, armor_color * 1.05F, nullptr, 1.0F,
           1});

  QMatrix4x4 lower_plate = chest.make_local_transform(
      ctx.model, QVector3D(0.0F, -0.12F, 0.05F), 0.85F);
  lower_plate.scale(0.38F, 0.18F, 0.32F);
  batch.meshes.push_back({get_unit_sphere(), nullptr, lower_plate, armor_color * 0.95F, nullptr, 1.0F,
           1});
}
