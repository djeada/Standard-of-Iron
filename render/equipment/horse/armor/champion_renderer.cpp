...
auto champion_archetype() -> const RenderArchetype& {
  static const RenderArchetype archetype = [] {
    ...
    builder.add_palette_mesh(
        get_unit_sphere(),
        box_local_model(QVector3D(0.0F, 0.0F, 0.0F), QVector3D(0.42F, 0.35F, 0.38F)),
        k_body_slot,
        nullptr,
        1.0F,
        1);
    builder.add_palette_mesh(
        get_unit_sphere(),
        box_local_model(QVector3D(0.0F, 0.12F, 0.05F), QVector3D(0.38F, 0.18F, 0.32F)),
        k_trim_slot,
        nullptr,
        1.0F,
        1);
...
auto champion_fill_role_colors(const HorseVariant& variant,
                               QVector3D* out,
...
auto champion_make_static_attachment(std::uint16_t socket_bone_index,
                                     std::uint8_t base_role_byte,
                                     const HorseAttachmentFrame& bind_pose_frame,
                                     const QMatrix4x4& bind_palette_socket_bone)
    -> Render::Creature::StaticAttachmentSpec {
  auto spec = make_static_attachment_spec_from_horse_renderer(champion_archetype(),
                                                              socket_bone_index,
                                                              bind_pose_frame,
...
void ChampionRenderer::submit(const DrawContext& ctx,
  ...
  append_horse_attachment_archetype(
      batch, ctx, frames.chest, champion_archetype(), palette);
}
