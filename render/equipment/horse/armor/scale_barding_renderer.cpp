...
void ScaleBardingRenderer::render(const DrawContext &ctx,
  ...
  QMatrix4x4 chest_armor =
      chest.makeLocalTransform(ctx.model, QVector3D(0.0F, -0.05F, 0.0F), 1.0F);
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    QMatrix4x4 side_armor = barrel.makeLocalTransform(
  ...
  QMatrix4x4 neck_armor =
      neck.makeLocalTransform(ctx.model, QVector3D(0.0F, 0.0F, 0.15F), 1.0F);
  neck_armor.scale(0.36F, 0.30F, 0.38F);
  out.mesh(getUnitSphere(), neck_armor, armor_color, nullptr, 1.0F);
}
