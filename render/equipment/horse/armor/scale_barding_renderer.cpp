...
auto scale_barding_chest_archetype() -> const RenderArchetype& {
  static const RenderArchetype archetype = [] {
    ...
    builder.add_palette_mesh(
        get_unit_sphere(),
        box_local_model(QVector3D(0.0F, -0.05F, 0.0F), QVector3D(0.40F, 0.32F, 0.35F)),
        k_barding_slot,
        nullptr,
        1.0F,
        1);
...
auto scale_barding_barrel_archetype() -> const RenderArchetype& {
  static const RenderArchetype archetype = [] {
    ...
    builder.add_palette_mesh(
        get_unit_sphere(),
        box_local_model(QVector3D(0.35F, -0.10F, 0.0F), QVector3D(0.12F, 0.28F, 0.48F)),
        k_barding_slot,
        nullptr,
        1.0F,
        1);
...
auto scale_barding_neck_archetype() -> const RenderArchetype& {
  static const RenderArchetype archetype = [] {
    ...
    builder.add_palette_mesh(
        get_unit_sphere(),
        box_local_model(QVector3D(0.0F, 0.0F, 0.15F), QVector3D(0.36F, 0.30F, 0.38F)),
        k_barding_slot,
        nullptr,
        1.0F,
        1);
...
auto scale_barding_fill_role_colors(const HorseVariant& variant,
                                    QVector3D* out,
...
auto scale_barding_make_static_attachment(const RenderArchetype& archetype,
...
void ScaleBardingRenderer::submit(const DrawContext& ctx,
  ...
  append_horse_attachment_archetype(
      batch, ctx, frames.chest, scale_barding_chest_archetype(), palette);
  append_horse_attachment_archetype(
      batch, ctx, frames.barrel, scale_barding_barrel_archetype(), palette);
  append_horse_attachment_archetype(
      batch, ctx, frames.neck_base, scale_barding_neck_archetype(), palette);
}
