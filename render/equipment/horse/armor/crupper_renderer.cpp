...
auto crupper_archetype() -> const RenderArchetype& {
  static const RenderArchetype archetype = [] {
    ...
    builder.add_palette_mesh(
        get_unit_sphere(),
        box_local_model(QVector3D(0.0F, 0.02F, -0.15F), QVector3D(0.48F, 0.32F, 0.28F)),
        k_armor_slot,
        nullptr,
        1.0F,
        1);
...
auto crupper_make_static_attachment(std::uint16_t socket_bone_index,
  ...
  auto spec = make_static_attachment_spec_from_horse_renderer(crupper_archetype(),
                                                              socket_bone_index,
                                                              bind_pose_frame,
...
void CrupperRenderer::submit(const DrawContext& ctx,
  ...
  append_horse_attachment_archetype(
      batch, ctx, frames.rump, crupper_archetype(), palette);
}
