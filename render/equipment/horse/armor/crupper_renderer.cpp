...
void CrupperRenderer::render(const DrawContext &ctx,
  ...
  QMatrix4x4 rear_plate =
      rump.makeLocalTransform(ctx.model, QVector3D(0.0F, 0.02F, -0.15F), 1.0F);
  rear_plate.scale(0.48F, 0.32F, 0.28F);
  out.mesh(getUnitSphere(), rear_plate, armor_color, nullptr, 1.0F);

  for (int i = 0; i < 2; ++i) {
    float const side = (i == 0) ? 1.0F : -1.0F;
    QMatrix4x4 side_plate = rump.makeLocalTransform(
