...
void BoulderRenderer::set_light_direction(const QVector3D &dir) {
  m_light_direction = dir.isNull() ? StoneBatchParams::default_light_direction()
                                   : dir.normalized();
  m_state.params.light_direction = m_light_direction;
}
...
void BoulderRenderer::submit(Renderer &renderer, ResourceManager *resources) {
  ...
  const auto visible_count = Scatter::sync_filtered_state(
      m_state, [](const StoneInstanceGpu &inst) -> const QVector4D & {
...
void BoulderRenderer::clear() { m_state.reset_instances(); }
...
void BoulderRenderer::generate_instances(
  ...
  terrain_cache.build_from_height_map(height_map.get_height_data(),
                                      height_map.getTerrainTypes(), width,
  ...
  ScatterCompositionContext composition(terrain_cache, width, map_height,
                                        tile_size, m_biome_settings,
                                        world_props);

  auto add_boulder = [&](float gx, float gz, float scale_min, float scale_max,
                         uint32_t &state) -> bool {
    ...
    float const brown_mix =
        remap(rand_01(state), 0.08F, 0.28F + scene.dryness * 0.12F);
    ...
    inst.color_rot = QVector4D(color.x(), color.y(), color.z(),
                               rand_01(state) * MathConstants::k_two_pi);
  ...
  for (const auto &prop : world_props) {
    ...
    uint32_t state =
        hash_coords(static_cast<int>(prop.x), static_cast<int>(prop.z),
