...
void OliveRenderer::generate_olive_instances() {
...

  {
    ...
    for (const auto &prop : m_world_props) {
      ...
      inst.rotation =
          QVector4D(prop.rotation, silhouette_seed, leaf_seed, bark_seed);
  ...
  if (scatter_profile.plant_density > 0.0F) {
    olive_density =
        scatter_profile.plant_density * scatter_rules.olive_density_scale;
  }
  ...
  auto add_olive = [&](float gx, float gz, uint32_t &state) -> bool {
    ...
    if (rand_01(state) >
        scatter_spawn_chance(ScatterRuleSpecies::Olive, scene)) {
      return false;
    }
    ...
    float const gray_mix =
        remap(rand_01(state), 0.08F + scene.rockiness * 0.04F,
              0.18F + scene.dryness * 0.08F);
    ...
    float const chosen_scale =
        remap(rand_01(state), scatter_rules.olive_scale_min,
              scatter_rules.olive_scale_max) *
        tile_safe * scatter_scale_bias(ScatterRuleSpecies::Olive, scene);
  ...
  for (int z = 0; z < m_height; z += k_tree_cell_span) {
    for (int x = 0; x < m_width; x += k_tree_cell_span) {
      ...
      density_mult *=
          scatter_density_multiplier(ScatterRuleSpecies::Olive, cell_scene);

      float const effective_density =
          olive_density * density_mult *
      ...
      for (int i = 0; i < olive_count; ++i) {
        ...
        auto const leader_scene =
            composition.sample_grid(gx, gz, state ^ 0x0E63A1B2U);
