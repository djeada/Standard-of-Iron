...
class SupplyCartRenderer : public IRenderPass {
public:
  SupplyCartRenderer();
  ~SupplyCartRenderer() override;
...
private:
  ...
  QVector3D m_light_direction = SupplyCartBatchParams::default_light_direction();

  Render::Ground::Scatter::FilteredRendererState<SupplyCartInstanceGpu,
                                                 SupplyCartBatchParams>
      m_state;
};
