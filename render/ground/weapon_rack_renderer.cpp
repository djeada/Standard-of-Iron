...
void WeaponRackRenderer::configure(
  ...
  m_state.params.light_direction = WeaponRackBatchParams::default_light_direction();
...
void WeaponRackRenderer::submit(Renderer &renderer, ResourceManager *resources) {
  ...
  const auto visible_count = Scatter::sync_filtered_state(
      m_state,
      [](const WeaponRackInstanceGpu &inst) -> const QVector4D & {
...
void WeaponRackRenderer::clear() { m_state.reset_instances(); }
