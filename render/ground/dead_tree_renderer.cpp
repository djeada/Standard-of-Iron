...
constexpr float k_ring_distance_scale = 2.0F;
...
void DeadTreeRenderer::configure(
  ...
  m_state.params.light_direction = DeadTreeBatchParams::default_light_direction();
...
void DeadTreeRenderer::submit(Renderer &renderer, ResourceManager *resources) {
  ...
  const auto visible_count = Scatter::sync_filtered_state(
      m_state,
      [](const DeadTreeInstanceGpu &inst) -> const QVector4D & {
