...
NATIONS = {"roman": "roman_republic", "carthage": "carthage"}
...
def check_style_configs():
    ...
    for troop in TROOPS:
        for nation_short, nation_full in NATIONS.items():
            ...
            if not matches:
                issues.append(
                    f"{nation_short}/{troop}: No shader_id found in {style_file}"
                )
...
def main():
    ...
    if total_issues == 0:
        ...
        print()
        print(
            f"All {total_combinations} nation-troop combinations ({len(NATIONS)} nations × {len(TROOPS)} troop types) are properly configured."
        )
        return 0
    else:
