...
def check_qt() -> Tuple[bool, Optional[Path], Optional[str]]:
    ...
    if qt_path:
        ...
        if qmake.exists():
            try:
                result = run_command(
                    [str(qmake), "-query", "QT_VERSION"], capture_output=True
                )
                version = result.stdout.strip()
    ...
    if qmake_exec and not mingw_qt:
        try:
            ...
            try:
                out = run_command(
                    [qmake_exec, "-query", "QT_VERSION"], capture_output=True
                )
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if result.returncode == 0 or result.returncode == 3010:
                ...
                error(
                    f"Visual Studio installation failed (exit code: {result.returncode})"
                )
                return False
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for qt_url in qt_urls:
                try:
                    info(
                        f"Downloading Qt online installer from {qt_url.split('/')[2]}..."
                    )
            ...
            print(
                f"{Color.BOLD}=== IMPORTANT: Follow these steps in the installer ==={Color.RESET}"
            )
            print()
            print(f"{Color.BOLD}1. Qt Account:{Color.RESET}")
            print("   - Login with your Qt account (or create one - it's free)")
            ...
            warning(
                "Complete the installation, then restart your terminal and run this script again"
            )
            print()
            return False
...
def auto_install_dependencies() -> bool:
    ...
    if not check_winget_available():
        ...
        warning(
            "Please install dependencies manually or update to Windows 10/11 with winget"
        )
        return False
    ...
    if success_count == len(installs_needed):
        ...
        if needs_restart:
            print()
            info(
                f"{Color.BOLD}IMPORTANT: CMake/Ninja were just installed!{Color.RESET}"
            )
...
def configure_project(
    build_dir: Path, build_type: str, qt_path: Optional[Path], msvc_env: dict
) -> None:
...
def build_project(build_dir: Path, msvc_env: dict) -> None:
    ...
    if parallel:
        ...
        warning(
            "  Using all CPU cores - set CMAKE_BUILD_PARALLEL_LEVEL=2 to reduce load in VMs"
        )
...
def create_package(build_dir: Path, build_type: str) -> Path:
    ...
    shutil.make_archive(package_path.stem, "zip", app_dir)
...
def main() -> int:
    ...
    parser.add_argument(
        "--no-package", action="store_true", help="Skip creating distributable package"
    )
        ...
        if not deps_ok:
            ...
            if not args.no_auto_install:
                ...
                if auto_install_dependencies():
                    warning(
                        "Dependencies installed - please restart your terminal and run this script again"
                    )
                    return 1
                else:
                    print()
    ...
    if not args.no_package:
        ...
        info(f"Executable: {build_dir / 'bin' / 'standard_of_iron.exe'}")
        info(
            "Run with debug scripts: run_debug.cmd, run_debug_softwaregl.cmd, run_debug_angle.cmd"
        )
