...
TEST_F(SerializationTest, EntityDeserializationRoundTrip) {
  ...
  auto* deserialized_transform = new_entity->get_component<TransformComponent>();
...
TEST_F(SerializationTest, WorldSerializationPreservesScatterSources) {
  ...
  EXPECT_EQ(terrain.world_props().back().type, Game::Map::WorldProp::Type::DeadTree);
...
TEST_F(SerializationTest, WorldSerializationPreservesCursedGoldVeinProp) {
  Game::Map::MapDefinition map_def;
  map_def.grid.width = 5;
  map_def.grid.height = 5;
  map_def.grid.tile_size = 1.0F;
  map_def.world_props.push_back({.type = Game::Map::WorldProp::Type::CursedGoldVein,
                                 .x = 1.0F,
                                 .z = 3.0F,
                                 .scale = 1.1F,
                                 .rotation = 0.25F});
  ...
  EXPECT_FLOAT_EQ(terrain.world_props().front().x, 1.0F);
  EXPECT_FLOAT_EQ(terrain.world_props().front().z, 3.0F);
  EXPECT_FLOAT_EQ(terrain.world_props().front().scale, 1.1F);
}
...
TEST_F(SerializationTest, CommanderComponentRoundTrip) {
  ...
  EXPECT_FLOAT_EQ(deserialized->aura_morale_bonus, commander->aura_morale_bonus);
  ...
  EXPECT_FLOAT_EQ(deserialized->rally_morale_restore, commander->rally_morale_restore);
  EXPECT_FLOAT_EQ(deserialized->rally_cooldown_remaining,
                  commander->rally_cooldown_remaining);
  EXPECT_FLOAT_EQ(deserialized->rally_feedback_time, commander->rally_feedback_time);
  EXPECT_FLOAT_EQ(deserialized->death_shock_radius, commander->death_shock_radius);
...
TEST_F(SerializationTest, MoraleComponentRoundTrip) {
  ...
  EXPECT_FLOAT_EQ(deserialized->commander_aura_bonus, morale->commander_aura_bonus);
...
TEST_F(SerializationTest, GuardModeComponentSerialization) {
  ...
  EXPECT_EQ(guard_mode_obj["guarded_entity_id"].toVariant().toULongLong(), 42ULL);
...
TEST_F(SerializationTest, CatapultLoadingComponentRoundTrip) {
  ...
  ASSERT_NE(deserialized, nullptr);
  EXPECT_EQ(deserialized->state, CatapultLoadingComponent::LoadingState::ReadyToFire);
...
TEST_F(SerializationTest, CombatStatePreserved) {
  ...
  auto* restored_attacker_attack = restored_attacker->get_component<AttackComponent>();
  ...
  EXPECT_EQ(restored_attacker_attack->current_mode, AttackComponent::CombatMode::Melee);
  ...
  auto* restored_defender_attack = restored_defender->get_component<AttackComponent>();
  ...
  auto* restored_attacker_unit = restored_attacker->get_component<UnitComponent>();
  auto* restored_defender_unit = restored_defender->get_component<UnitComponent>();
...
TEST_F(SerializationTest, NationIdentityPreserved) {
  ...
  EXPECT_EQ(restored_roman_comp->nation_id, Game::Systems::NationID::RomanRepublic);
  EXPECT_EQ(restored_roman_comp->spawn_type, Game::Units::SpawnType::Spearman);

  auto* restored_carthage_comp = restored_carthage->get_component<UnitComponent>();
...
TEST_F(SerializationTest, ElephantComponentRoundTrip) {
  ...
  ASSERT_NE(deserialized, nullptr);
  EXPECT_EQ(deserialized->charge_state, ElephantComponent::ChargeState::Trampling);
...
TEST_F(SerializationTest, ElephantStompImpactComponentRoundTrip) {
  auto* original_entity = world->create_entity();
  auto* stomp_impact = original_entity->add_component<ElephantStompImpactComponent>();
  ...
  auto* deserialized = new_entity->get_component<ElephantStompImpactComponent>();
  ASSERT_NE(deserialized, nullptr);
