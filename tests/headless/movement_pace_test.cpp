...
TEST_F(MovementPaceTest, AnArmyReorderedThroughATownHoldsPace) {
  ...
  for (auto const& record : m_records) {
    ...
    expect_clean(record, 12, 90, 0.20F, 60);
  }

  order(army, world_of(58, 30));
  run_for(0.1);
  double arrival_budget = 0.0;
  for (auto const id : army) {
    ...
    arrival_budget =
        std::max(arrival_budget,
                 static_cast<double>(facts->progress.remaining_arclength /
                                     std::max(0.1F, facts->desired.speed_limit)) *
                         1.25 +
                     2.0);
  }
  run_for(arrival_budget);
