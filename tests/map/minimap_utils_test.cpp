...
TEST_F(MinimapUtilsTest, PixelToWorldInversesWorldToPixel) {
  ...
  for (const auto& [world_x, world_z] : test_coords) {
    ...
    auto [px, py] = world_to_pixel(
        grid_x, grid_z, world_width, world_height, img_width, img_height);
...
TEST_F(MinimapUtilsTest, CenterPixelMapsToCenterWorld) {
  ...
  auto [world_x, world_z] = pixel_to_world(center_px,
                                           center_py,
                                           world_width,
                                           world_height,
                                           img_width,
...
TEST_F(MinimapUtilsTest, CornerPixelsMapToExpectedWorldCoords) {
  ...
  auto [tl_x, tl_z] = pixel_to_world(
      0.0F, 0.0F, world_width, world_height, img_width, img_height, TILE_SIZE);

  auto [br_x, br_z] = pixel_to_world(img_width,
                                     img_height,
                                     world_width,
                                     world_height,
                                     img_width,
...
TEST_F(MinimapUtilsTest, SquareMapSymmetry) {
  ...
  auto [w1_x, w1_z] = pixel_to_world(quarter_x,
                                     quarter_y,
                                     world_width,
                                     world_height,
                                     img_width,
                                     img_height,
                                     TILE_SIZE);
  auto [w2_x, w2_z] = pixel_to_world(three_quarter_x,
                                     three_quarter_y,
                                     world_width,
                                     world_height,
                                     img_width,
  ...
  auto [p1_x, p1_y] =
      world_to_pixel(g1_x, g1_z, world_width, world_height, img_width, img_height);
  ExpectNearPair({p1_x, p1_y}, {quarter_x, quarter_y});

  const float g2_x = w2_x / TILE_SIZE;
  const float g2_z = w2_z / TILE_SIZE;
  auto [p2_x, p2_y] =
      world_to_pixel(g2_x, g2_z, world_width, world_height, img_width, img_height);
...
TEST_F(MinimapUtilsTest, TileSizeScaling) {
  ...
  auto [px, py] =
      world_to_pixel(grid_x, grid_z, world_width, world_height, img_width, img_height);

  auto [world_x, world_z] = pixel_to_world(
      px, py, world_width, world_height, img_width, img_height, tile_size);
