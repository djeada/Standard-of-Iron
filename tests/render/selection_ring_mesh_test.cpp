...
TEST(SelectionRingMesh, FacesUpwardForOverheadRendering) {
  ...
  float const first_tri_y =
      triangle_normal_y(vertices[indices[0]], vertices[indices[1]],
                        vertices[indices[2]]);
  float const second_tri_y =
      triangle_normal_y(vertices[indices[3]], vertices[indices[4]],
                        vertices[indices[5]]);
