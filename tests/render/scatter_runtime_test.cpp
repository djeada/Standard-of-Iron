...
auto make_snapshot() -> Game::Map::VisibilityService::Snapshot {
  ...
  snapshot.cells.assign(16, static_cast<std::uint8_t>(Game::Map::VisibilityState::Unseen));
  snapshot.cells[10] =
...
TEST(ScatterRuntimeTest, DirectGpuReadyAllowsEmptyScatterBuffers) {
  ...
  EXPECT_TRUE(
      Render::Ground::Scatter::is_direct_gpu_ready(instances, buffer));

  instances.push_back({QVector4D(0.0F, 0.0F, 0.0F, 1.0F),
