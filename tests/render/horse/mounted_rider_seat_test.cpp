...
TEST(MountedRiderSeatTest, SeatAndStirrupsSitOnTheHorse) {
  ...
  std::cout << "saddle_center y=" << mount.saddle_center.y()
            ...
            << "body_width=" << dims.body_width << "  stirrup_out=" << dims.stirrup_out
  ...
  float const straight_leg =
      HumanProportions::UPPER_LEG_LEN + HumanProportions::LOWER_LEG_LEN;
...
TEST(MountedRiderSeatTest, RiderSitsAtABelievableHeight) {
  ...
  float const seated_head_y = mount.seat_position.y() + (HumanProportions::HEAD_TOP_Y -
                                                         HumanProportions::WAIST_Y);
