...
public:
  ...
  void cylinder(const QVector3D &, const QVector3D &, float, const QVector3D &,
                float) override { ++total_calls; }
  void selection_ring(const QMatrix4x4 &, float, float,
                      const QVector3D &) override { ++total_calls; }
  void grid(const QMatrix4x4 &, const QVector3D &, float, float,
            float) override { ++total_calls; }
  void selection_smoke(const QMatrix4x4 &, const QVector3D &, float) override { ++total_calls; }
  void healing_beam(const QVector3D &, const QVector3D &, const QVector3D &,
                    float, float, float, float) override { ++total_calls; }
...
TEST(TemplatePrewarmRegression, AllLodLevelsRespectPrewarmFiltering) {
  ...
  // Test all LOD levels
  for (auto lod : {CreatureLOD::Full, CreatureLOD::Reduced, CreatureLOD::Minimal}) {
    PreparedCreatureRenderRow row{};
    row.spec.kind = CreatureKind::Humanoid;
    row.lod = lod;
    row.pass = pass_intent_from_ctx(prewarm_ctx);
