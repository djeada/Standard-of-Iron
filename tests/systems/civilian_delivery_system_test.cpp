...
TEST(CivilianDeliverySystemTest,
     DeliveryTargetOutsideBarracksStillTransfersManpower) {
  ...
  collision_registry.register_building(
      barracks->get_id(), "barracks", barracks_transform->position.x,
      barracks_transform->position.z, barracks_unit->owner_id);
  pathfinder->update_building_obstacles();

  float const civilian_radius = 0.5F;
  QVector3D const delivery_target =
      App::Utils::barracks_delivery_target_position(
          QVector3D(12.0F, 0.0F, 20.0F),
          QVector3D(barracks_transform->position.x, 0.0F,
  ...
  civilian_transform->position = {delivery_target.x(), 0.0F,
                                  delivery_target.z()};
  civilian_unit->spawn_type = Game::Units::SpawnType::Civilian;
  civilian_unit->owner_id = 1;
  delivery->target_barracks_id = barracks->get_id();
