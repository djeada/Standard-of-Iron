...
Button {
    ...
    contentItem: Text {
        text: control.text
        color: !control.enabled ? Design.Theme.textDisabled :
               control.destructive ? Design.Theme.danger : Design.Theme.textPrimary
    ...
    background: Rectangle {
        color: !control.enabled ? Design.Theme.panelIron :
               control.down ? Qt.darker(Design.Theme.accent, 1.35) :
               control.hovered ? Design.Theme.panelLeather :
               control.primary ? Qt.darker(Design.Theme.accent, 1.65) :
               Design.Theme.panelIron
        radius: Design.Metrics.radiusSmall
        border.width: control.activeFocus ? Design.Metrics.borderFocus : Design.Metrics.borderThin
        border.color: !control.enabled ? Design.Theme.borderSubtle :
                      control.destructive ? Design.Theme.danger :
                      control.activeFocus ? Design.Theme.focus :
                      control.hovered ? Design.Theme.accent : Design.Theme.borderStrong

        Behavior on color {
            ColorAnimation { duration: Design.Motion.fast }
        }
    }
}
