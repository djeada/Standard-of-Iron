...
- [ ] The selection ring is blue, which in a two-player match is the _enemy's_
...
- [ ] **Stray road-mesh triangles float above the terrain.** Two sharp tan shards,
      ...
      samples and takes the _max_ of five height taps, so it ought to sit above
...
_look_: no parapets, no piers, a warped deck whose ends do not meet the road, and
...
- [ ] Cache the mastering _analysis_ by asset id for the process lifetime and
...
- Fog of war _logic_ — reveal on movement, explored persists, `reveal_all` in
