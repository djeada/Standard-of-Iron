...
Tagged releases publish a package for each platform on the
[Releases page](https://github.com/djeada/Standard-of-Iron/releases). Every
package is built and smoke-tested through a real OpenGL self-test before it is
attached, and each ships with a `.sha256` companion you can verify.

| Platform | File                                               | To run it                                                                      |
| -------- | -------------------------------------------------- | ------------------------------------------------------------------------------ |
| Linux    | `standard_of_iron-<version>-linux-x86_64.AppImage` | `chmod +x` the file, then run it. No installation, no root.                    |
| macOS    | `standard_of_iron-<version>-macos-universal.dmg`   | Open the disk image and drag the app across. See the note below.               |
| Windows  | `standard_of_iron-<version>-win-x64.zip`           | Unpack anywhere and run `standard_of_iron.exe`. Everything it needs is inside. |

To check a download against its checksum:

```bash
sha256sum --check --strict standard_of_iron-0.1.0-linux-x86_64.AppImage.sha256
```

**macOS builds are universal** — Intel and Apple Silicon both run natively, with
no Rosetta translation. The build takes its architectures from whatever the
installed Qt provides and fails if the binary does not come out matching, so the
name on the download says what is actually inside it.

Apple has deprecated OpenGL and caps it at 4.1, which is still above the 3.3 Core
floor the renderer needs. The optional GPU crowd-culling path wants 4.3 and
switches itself off there; everything else behaves as it does on Linux and
Windows.

Builds are unsigned unless the release was made with notarization credentials
configured, so Gatekeeper will ask you to confirm the first launch.

Prefer to build it yourself? Skip to [Build Instructions](#build-instructions).
...
- Audio playback uses _miniaudio_ (public domain / MIT-0) and _stb_vorbis_ (public domain / MIT). Both are vendored under `third_party/`.
- Voice lines were recorded by the project author and are covered by the MIT licence above.

**The music is the one exception to MIT.** The twenty tracks under `assets/audio/music/` were generated locally with [Meta's AudioCraft](https://github.com/facebookresearch/audiocraft), whose model weights are licensed [CC BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/) — non-commercial use only. Standard of Iron is given away free of charge, which is consistent with that; **selling the game, or bundling it into anything sold, would require replacing those tracks first.**

Every sound effect is either generated by this repository's own code or built from CC0 recordings, and carries a `source` tag in the audio manifest saying which. See [THIRD_PARTY_LICENSES.md](https://github.com/djeada/Standard-of-Iron/blob/main/THIRD_PARTY_LICENSES.md) for the per-asset record.
