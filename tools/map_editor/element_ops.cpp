...
auto count(const MapData& data, int kind) -> int {
  ...
  case ElementKind::Forest:
    return static_cast<int>(data.forests().size());
  }
  return 0;
}
...
auto type_name(const ElementSnapshot& snap) -> QString {
  return std::visit(Overloaded{
                        ...
                        [](const ForestElement&) { return QStringLiteral("forest"); },
...
auto display_name(const ElementSnapshot& snap) -> QString {
  ...
  if (const auto* forest = std::get_if<ForestElement>(&snap)) {
    return forest->id.isEmpty() ? QStringLiteral("forest") : forest->id;
  }
...
auto summary(const ElementSnapshot& snap) -> QString {
  ...
  } else if (const auto* forest = std::get_if<ForestElement>(&snap)) {
    text += QStringLiteral("\nradius %1")
                .arg(static_cast<double>(forest->radius), 0, 'f', 1);
  }
...
void apply(MapData& data, int index, const ElementSnapshot& snap) {
  std::visit(
      Overloaded{
          ...
          [&](const ForestElement& e) { data.update_forest(index, e); },
      },
      snap);
}
...
auto make_add(MapData& data, const ElementSnapshot& snap) -> std::unique_ptr<Command> {
  return std::visit(Overloaded{
                        ...
                        [&](const ForestElement& e) -> std::unique_ptr<Command> {
                          return std::make_unique<AddForestCmd>(&data, e);
                        },
                    },
                    snap);
}
...
auto make_remove(MapData& data, int kind, int index) -> std::unique_ptr<Command> {
  ...
  case ElementKind::Forest:
    return std::make_unique<RemoveForestCmd>(&data, index, data.forests()[index]);
  }
  return {};
}
...
auto make_update(MapData& data,
  ...
  return std::visit(
      Overloaded{
          ...
          [&](const ForestElement& e) -> std::unique_ptr<Command> {
            return std::make_unique<UpdateForestCmd>(
                &data, index, std::get<ForestElement>(before), e, description);
          },
      },
      after);
}
