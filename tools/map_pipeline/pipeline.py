...
def write_geojson(geoms: Iterable, out_path: Path):
  features = [{"type": "Feature", "geometry": mapping(geom), "properties": {}} for geom in geoms]
...
def clip_project_lines(geoms: Iterable, bounds: MapBounds) -> List[List[Tuple[float, float]]]:
  bbox = bounds.clip_box()
  lines: List[List[Tuple[float, float]]] = []
  for geom in geoms:
    clipped = geom.intersection(bbox)
...
def draw_lines(draw: ImageDraw.ImageDraw, lines, width: int, height: int, color, stroke: int):
  for line in lines:
...
def render_preview(uv_geoms, coast_lines, river_lines, size: int = 4096, oversample: int = 2):
  w = h = size * oversample
...
def render_textures(uv_geoms, coast_lines, river_lines, size: int = 4096, oversample: int = 2):
  w = h = size * oversample
  sea_top = (54, 90, 120)
  sea_bottom = (28, 58, 82)
  ...
  base.resize((size, size), Image.Resampling.LANCZOS).save(ASSETS_DIR / "campaign_base_color.png")
  water.resize((size, size), Image.Resampling.LANCZOS).save(ASSETS_DIR / "campaign_water.png")
...
def main():
  ...
  logging.info("Bounds: lon %.2f..%.2f lat %.2f..%.2f", bounds.lon_min, bounds.lon_max, bounds.lat_min, bounds.lat_max)
