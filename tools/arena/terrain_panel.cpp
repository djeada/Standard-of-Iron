...
void bind_slider_to_double(QSlider* slider,
  ...
  QObject::connect(
      slider, &QSlider::valueChanged, spin_box, [spin_box, scale](int value) {
        const QSignalBlocker blocker(spin_box);
        spin_box->setValue(static_cast<double>(value) / scale);
      });
  QObject::connect(
      slider, &QSlider::valueChanged, slider, [scale, emit_value](int value) {
  ...
  QObject::connect(spin_box,
                   qOverload<double>(&QDoubleSpinBox::valueChanged),
                   spin_box,
                   [emit_value](double value) { emit_value(value); });
}
...
TerrainPanel::TerrainPanel(QWidget* parent)
  ...
  connect(regenerate_button,
          &QPushButton::clicked,
          this,
          &TerrainPanel::regenerate_requested);
  connect(wireframe_box, &QCheckBox::toggled, this, &TerrainPanel::wireframe_toggled);
  connect(normals_box, &QCheckBox::toggled, this, &TerrainPanel::normals_toggled);
  connect(ground_type_box,
  ...
  bind_slider_to_double(frequency_slider, frequency_spin, 100.0, [this](double value) {
