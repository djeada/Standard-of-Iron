...
public:
  ...
  auto initialize(int music_channels = AudioConstants::DEFAULT_MUSIC_CHANNELS)
      -> bool;
  void shutdown();

  void register_track(const std::string &track_id,
  ...
  void set_volume(int channel, float volume,
                  int fade_ms = AudioConstants::NO_FADE_MS);
