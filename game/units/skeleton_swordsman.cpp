...
SkeletonSwordsman::SkeletonSwordsman(Engine::Core::World& world)
    : Unit(world, TroopType::SkeletonSwordsman) {
}

auto SkeletonSwordsman::Create(Engine::Core::World& world, const SpawnParams& params)
    -> std::unique_ptr<SkeletonSwordsman> {
  auto unit = std::unique_ptr<SkeletonSwordsman>(new SkeletonSwordsman(world));
  unit->init(params);
  return unit;
}

void SkeletonSwordsman::init(const SpawnParams& params) {
  auto* e = m_world->create_entity();
  m_id = e->get_id();

  const auto nation_id = resolve_nation_id(params);
  const auto troop_type =
      spawn_typeToTroopType(params.spawn_type).value_or(TroopType::SkeletonSwordsman);
  auto profile =
