...
SkeletonArcher::SkeletonArcher(Engine::Core::World& world)
    : Unit(world, TroopType::SkeletonArcher) {
}

auto SkeletonArcher::Create(Engine::Core::World& world, const SpawnParams& params)
    -> std::unique_ptr<SkeletonArcher> {
  auto unit = std::unique_ptr<SkeletonArcher>(new SkeletonArcher(world));
  unit->init(params);
  return unit;
}

void SkeletonArcher::init(const SpawnParams& params) {
  auto* e = m_world->create_entity();
  m_id = e->get_id();

  const auto nation_id = resolve_nation_id(params);
  const auto troop_type =
      spawn_typeToTroopType(params.spawn_type).value_or(TroopType::SkeletonArcher);
  auto profile =
  ...
  if (profile.has_ability("cursed_arrow_volley")) {
