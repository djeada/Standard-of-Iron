...
[[nodiscard]] auto
distance_sq(const Engine::Core::TransformComponent &a,
...
auto morale_for(Engine::Core::Entity *entity)
    -> Engine::Core::MoraleComponent * {
...
void apply_commander_death_shock(
    Engine::Core::World *world, Engine::Core::Entity *commander_entity,
    Engine::Core::CommanderComponent &commander,
    const Engine::Core::UnitComponent &unit,
...
void CommanderSystem::update(Engine::Core::World *world, float delta_time) {
  ...
  for (auto *entity :
       world->get_entities_with<Engine::Core::MoraleComponent>()) {
    auto *morale = entity->get_component<Engine::Core::MoraleComponent>();
    if (morale == nullptr) {
      continue;
    }
    ...
    for (auto *candidate :
      ...
      if (dist_sq <= aura_radius_sq) {
        if (candidate_is_troop) {
          ...
          if (morale != nullptr) {
            morale->commander_aura_bonus = std::max(
                morale->commander_aura_bonus, commander->aura_morale_bonus);
        ...
        if (candidate_is_troop && commander->bonus_type == "health_regen") {
          candidate_unit->health =
              std::min(candidate_unit->max_health,
                       candidate_unit->health +
                           static_cast<int>(std::round(
                               commander->aura_bonus_value * delta_time)));
        } else if (candidate_is_troop &&
          ...
          if (auto *production =
            ...
            if (production->in_progress) {
              ...
              production->time_remaining = std::max(
                  0.0F, production->time_remaining - delta_time * haste);
            }
          }
        }
      }
