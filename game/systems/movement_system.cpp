...
void MovementSystem::move_unit(Engine::Core::Entity* entity,
  ...
  if (have_target_velocity && facts->motor.has_contact) {

    float const nx = facts->motor.contact_nx;
    float const nz = facts->motor.contact_nz;
    float const into = (target_vx * nx) + (target_vz * nz);
    if (into < 0.0F) {
      float const wanted = std::hypot(target_vx, target_vz);
      float const along = std::hypot(target_vx - (nx * into), target_vz - (nz * into));
      if (along > 1.0e-4F && wanted > 1.0e-4F) {
        float const slide_x = (target_vx - (nx * into)) * (wanted / along);
        float const slide_z = (target_vz - (nz * into)) * (wanted / along);
        if (collision.step_allowed(old_x,
                                   old_z,
                                   old_x + (slide_x * delta_time),
                                   old_z + (slide_z * delta_time))) {
          target_vx = slide_x;
          target_vz = slide_z;
        }
      }
