...
void apply_visual_profile(ArrowInstance& arrow,
  ...
  case ArrowVisualStyle::Volley:
    ...
    arrow.roll_deg = remap_unit(unit_float(sequence ^ 0x55aa55aaU), -32.0F, 32.0F);
    arrow.spin_rate_deg =
        remap_unit(unit_float(sequence ^ 0xa55aa55aU), -280.0F, 280.0F);
    arrow.trail_alpha = remap_unit(unit_float(sequence ^ 0x0f1e2d3cU), 0.16F, 0.28F);
    arrow.trail_length = remap_unit(unit_float(sequence ^ 0x10293847U), 0.11F, 0.18F);
    arrow.brightness = remap_unit(unit_float(sequence ^ 0x56473829U), 0.88F, 1.08F);
    break;
  ...
  default:
    ...
    arrow.roll_deg = remap_unit(unit_float(sequence ^ 0x27182818U), -10.0F, 10.0F);
    arrow.spin_rate_deg = remap_unit(unit_float(sequence ^ 0x42424242U), -80.0F, 80.0F);
    arrow.trail_alpha = remap_unit(unit_float(sequence ^ 0xabcdef01U), 0.08F, 0.15F);
    arrow.trail_length = remap_unit(unit_float(sequence ^ 0x10fedcbaU), 0.06F, 0.10F);
    arrow.brightness = remap_unit(unit_float(sequence ^ 0xc001d00dU), 0.94F, 1.04F);
    break;
  }
}
