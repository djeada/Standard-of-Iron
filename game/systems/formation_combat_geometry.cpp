...
auto resolve_layout(const Engine::Core::Entity& entity) -> FormationLayout {
  ...
  if (rigid_body) {
    ...
    if (auto const* elephant =
            entity.get_component<Engine::Core::ElephantComponent>()) {
      result.body_radius = std::max(result.body_radius, elephant->trample_radius);
    }
...
auto contact_is_active(const Engine::Core::Entity& attacker,
  ...
  if (geometry.formation_overlap_required) {
    ...
    return deep_front_rank_overlap || reinforcement_has_visible_overlap;
  }
