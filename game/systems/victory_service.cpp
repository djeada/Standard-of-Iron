...
void VictoryService::on_barrack_captured(const Engine::Core::BarrackCapturedEvent&) {
...
auto VictoryService::check_elimination(Engine::Core::World& world) -> bool {
  ...
  for (auto* e : entities) {
    ...
    QString const unit_type_str =
        QString::fromStdString(Game::Units::spawn_typeToString(unit->spawn_type));
    if (std::find(m_key_structures.begin(), m_key_structures.end(), unit_type_str) !=
...
auto VictoryService::check_control_structures(Engine::Core::World& world,
