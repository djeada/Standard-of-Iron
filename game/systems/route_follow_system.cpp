...
void RouteFollowSystem::follow(Engine::Core::Entity& entity,
  ...
  if (route.valid()) {
    ...
    float const chord_x = aim_x - route_origin.first;
    float const chord_z = aim_z - route_origin.second;
    float const chord_length = std::hypot(chord_x, chord_z);
    auto const tangent = route.tangent_at(s);
    tangent_x = chord_length > 0.001F ? chord_x / chord_length : tangent.first;
    tangent_z = chord_length > 0.001F ? chord_z / chord_length : tangent.second;
  } else {
  ...
  if (goal_is_held_by_friends) {
    settle_arrival(entity, *movement, *facts, route, true);
    return;
  }
  ...
  if (current_position_allowed && remaining <= arrive_radius &&
    ...
    settle_arrival(entity, *movement, *facts, route, false);

    auto* guard_mode = world.try_get<Engine::Core::GuardModeComponent>(id);
