...
private:
  ...
  static void draw_mountain_symbol(
      QPainter& painter, float cx, float cy, float width, float height);
  static void
  draw_hill_symbol(QPainter& painter, float cx, float cy, float width, float height);
  static void
  ...
  [[nodiscard]] auto world_to_pixel_size(float world_size,
                                         const GridDefinition& grid) const -> float;

  [[nodiscard]] static auto biome_to_base_color(const BiomeSettings& biome) -> QColor;
