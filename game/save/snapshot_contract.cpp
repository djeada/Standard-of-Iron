...
constexpr std::array k_fields = std::to_array<FieldSpec>({
    ...
    {"SquadReformComponent",
     PresentationOnly,
     "Where the men of a just split or joined squad walk from to their new slots."},
    ...
    {"StructureRepairPresentationComponent",
     PresentationOnly,
     "Repair scaffolding and dust; a loaded crew raises it again."},
    ...
    {"ProductionCompletionComponent",
     PresentationOnly,
     "Short-lived glow on a unit that just arrived: a recruit leaving a "
     "building, or a guardian an Iron Sepulcher wave raised. It is a countdown "
     "the systems that spawn those units re-attach, so a restored match simply "
     "shows no glow rather than a stale one."},
    {"StockpileComponent",
     PresentationOnly,
